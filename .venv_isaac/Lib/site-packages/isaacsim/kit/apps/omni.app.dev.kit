[package]
title = "Kit Dev App"
description = "Default app kit runs. Minimal USD viewer."
version = "1.0.0"
keywords = ["app"]
support_level = "Sample"

[dependencies]
# The Main UI App. Bring everything needed for UI application.
"omni.kit.uiapp" = {}

# USD file open/close
"omni.kit.window.file" = {}

# Basic Menus
"omni.kit.menu.utils" = {}
"omni.kit.menu.file" = {}
"omni.kit.menu.edit" = {}
"omni.kit.menu.create" = {}
"omni.kit.menu.common" = {}
"omni.kit.menu.aov" = {}
"omni.kit.context_menu" = {}
"omni.kit.selection" = {}

# Stage Templates
"omni.kit.stage_templates" = {}

# Some Editing Windows
"omni.kit.window.title" = {}
"omni.kit.widget.layers" = {}
"omni.kit.widget.live" = {}
"omni.kit.widget.cache_indicator" = {}
"omni.kit.window.stage" = {}
"omni.kit.window.toolbar" = {}
"omni.kit.widget.filebrowser" = {}
"omni.kit.window.filepicker" = {}
"omni.kit.window.content_browser" = {}
"omni.kit.collaboration.channel_manager" = {}
"omni.kit.collaboration.viewport.camera" = {}
"omni.kit.collaboration.selection_outline" = {}
"omni.kit.collaboration.stage_columns" = {}
"omni.kit.stage_column.active" = {}

# Tools
"omni.kit.tool.collect" = {}

# Viewport and Renderer
"omni.kit.viewport.bundle" = {}
"omni.kit.viewport.rtx" = {}
"omni.kit.viewport.pxr" = {}
"omni.resourcemonitor" = {}

# MDL
"omni.mdl" = {}
"omni.mdl.neuraylib" = {}

# Usd MDL plugins
"omni.kit.usd.mdl" = {}

# RTX Tests
# "omni.rtx.tests" = {}
# "omni.kit.renderer.capture" = {}

# Status Bar
"omni.kit.window.status_bar" = {}

# Graphics Stats
"omni.stats" = {}

# Render Settings 2.0
"omni.rtx.settings.core" = {}

# Runtime Performance Monitor
"omni.rpm.ui" = {} # RPM UI for performance data visualization and benchmarking (metrics, RTX rendering, Render Graph etc.)

# Dev & Utils Windows
"omni.kit.window.about" = {}
"omni.kit.window.stats" = { order = 1000 }
"omni.kit.window.script_editor" = {}
"omni.kit.window.console" = {}
"omni.kit.window.preferences" = { order = 1000 } # so the menu is in the correct place
"omni.kit.window.tests" = {}
"omni.kit.profiler.window" = {}

# Extensions Manager UI
"omni.kit.window.extensions" = {}

# Property Widgets and Window
"omni.kit.window.property" = {}
"omni.kit.property.bundle" = {}

# Misc
"omni.kit.primitive.mesh" = {}
"omni.kit.telemetry" = {}

[settings.app.settings]
persistent = true

[settings]
exts."omni.kit.window.file".useNewFilePicker = true
exts."omni.kit.renderer.core".imgui.enableMips = true
app.content.emptyStageOnStart = true

# optional settings for RPM to enable specific features on start
exts."omni.rpm.core".enable = false
exts."omni.rpm.core".sampleRenderGraphData = false
exts."omni.rpm.core".renderPassSampling = false
exts."omni.rpm.core".renderPassSamplingMultiViewports = false
exts."omni.rpm.core".renderPassSamplingSubPassMode = false
exts."omni.rpm.core".enableDevelopmentMetrics = false
exts."omni.rpm.ui".enable = false
exts."omni.rpm.ui".showUi = false
exts."omni.rpm.ui".saveAllRenderGraphFrames = false

app.extensions.enabledDeprecated = []

renderer.enabled='rtx,pxr'
renderer.active='rtx'

# Sync materials in parallel from within Hydra
omnihydra.parallelHydraSprimSync = true

exts."omni.kit.viewport.window".startup.dockTabInvisible=true

[settings.app.window]
saveSizeOnExit = true
title = "Kit Dev App"
width = 1920
height = 1080
x = -1
y = -1
