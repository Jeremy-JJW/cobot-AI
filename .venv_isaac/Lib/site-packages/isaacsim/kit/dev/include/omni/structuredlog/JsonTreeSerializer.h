 ...
 */
static inline void ignoreJsonTreeSerializerValidationError([[maybe_unused]] const char* s)
{
}
...
template <bool validate = false,
          typename T,
          typename JsonSerializerType = JsonSerializer<validate, false, ignoreJsonTreeSerializerValidationError>,
          typename JsonNodeType = JsonNode,
          typename BlobReaderType = BlobReader<validate, ignoreJsonTreeSerializerValidationError>>
static inline bool serializeScalar(JsonSerializerType* serial, const JsonNodeType* root, T constVal, BlobReaderType* reader)
{
    ...
    {
        return serial->writeValueJson(root, constVal);
    }
    else
    {
        T b = {};
        ...
        return serial->writeValueJson(root, b);
    }
}
                                  ...
                                  BlobReaderType* reader)
{
    ...
    if ((root->flags & JsonNode::fFlagConst) != 0)
    {
        ...
        {
            result = serial->writeValueJson(root, constVal[i]);
            if (validate && !result)
                return false;
        }
    }
    else
    {
        ...
        {
            result = serial->writeValueJson(root, b[i]);
            if (validate && !result)
                return false;
        }
    }
...
static inline bool serializeEnum(JsonSerializerType* serial, const JsonNodeType* root, T* enumChoices, BlobReaderType* reader)
{
    ...
    return serial->writeValueJson(root, enumChoices[b]);
}
...
static inline bool serializeJsonTree(JsonSerializerType* serial, const JsonNodeType* root, BlobReaderType* reader)
{
    ...
    {
        case NodeType::eNull:
            return serial->writeValueJson(root);
        ...
        case NodeType::eBinary:
            if ((root->flags & JsonNode::fFlagConst) != 0)
            {
                return serial->writeValueWithBase64EncodingJson(root, root->data.binaryVal, root->len);
            }
            else
            {
                ...
                return serial->writeValueWithBase64EncodingJson(root, b, len);
            }
        ...
        case NodeType::eString:
            if ((root->flags & JsonNode::fFlagConst) != 0)
            {
                return serial->writeValueJson(root, root->data.strVal, (root->len == 0) ? 0 : root->len - 1);
            }
            else
            {
                ...
                // null terminator is included in the length
                return serial->writeValueJson(root, b, (len == 0) ? 0 : len - 1);
            }
        ...
        case NodeType::eStringArray:
            ...
            if ((root->flags & JsonNode::fFlagConst) != 0)
            {
                for (uint16_t i = 0; i < root->len; i++)
                {
                    result = serial->writeValueJson(root, root->data.strArrayVal[i]);
                    if (validate && !result)
                        return false;
                }
            }
...
/** Serialize JSON using a @ref JsonNode as the schema and a binary blob to read data.
 *
 *  This function exists specifically to support structured log serialization within
 *  omni.structuredlog and is tailored to that use case. The interface and implementation
 *  details are subject to change as needed by omni.structuredlog.
 *
 *  @param[inout] serial   The JSON serializer to serialize the data into.
...
static inline void serializeJsonSchema_(JsonSerializerType* serial, const JsonNodeType* root)
{
    ...
    serial->writeValueJson(root, nodeTypeString(root->type));

    serial->writeKey("flags");
    serial->writeValueJson(root, root->flags);
    ...
    if ((root->flags & JsonNode::fFlagConst) != 0)
    {
        ...
        {
            case NodeType::eNull:
                serial->writeValueJson(root);
                break;
            case NodeType::eBool:
                serial->writeValueJson(root, root->data.boolVal);
                break;
            case NodeType::eInt32:
                serial->writeValueJson(root, root->data.intVal);
                break;
            case NodeType::eUint32:
                serial->writeValueJson(root, root->data.uintVal);
                break;
            case NodeType::eInt64:
                serial->writeValueJson(root, root->data.intVal);
                break;
            case NodeType::eUint64:
                ...
                break;
            case NodeType::eFloat32:
                serial->writeValueJson(root, root->data.floatVal);
                break;
            case NodeType::eFloat64:
                serial->writeValueJson(root, root->data.floatVal);
                break;
            case NodeType::eBinary:
                serial->writeValueWithBase64EncodingJson(root, root->data.binaryVal, root->len);
                break;
            case NodeType::eBoolArray:
                serial->openArray();
                for (uint16_t i = 0; i < root->len; i++)
                {
                    serial->writeValueJson(root, root->data.boolArrayVal[i]);
                }
                serial->closeArray();
                break;
            case NodeType::eInt32Array:
                ...
                for (uint16_t i = 0; i < root->len; i++)
                {
                    serial->writeValueJson(root, root->data.int32ArrayVal[i]);
                }
                serial->closeArray();
                break;
            case NodeType::eUint32Array:
                ...
                for (uint16_t i = 0; i < root->len; i++)
                {
                    serial->writeValueJson(root, root->data.uint32ArrayVal[i]);
                }
                serial->closeArray();
                break;
            case NodeType::eInt64Array:
                ...
                for (uint16_t i = 0; i < root->len; i++)
                {
                    serial->writeValueJson(root, root->data.int64ArrayVal[i]);
                }
                serial->closeArray();
                break;
            case NodeType::eUint64Array:
                ...
                for (uint16_t i = 0; i < root->len; i++)
                {
                    serial->writeValueJson(root, root->data.uint64ArrayVal[i]);
                }
                serial->closeArray();
                break;
            case NodeType::eFloat32Array:
                ...
                for (uint16_t i = 0; i < root->len; i++)
                {
                    serial->writeValueJson(root, root->data.float32ArrayVal[i]);
                }
                serial->closeArray();
                break;
            case NodeType::eFloat64Array:
                ...
                for (uint16_t i = 0; i < root->len; i++)
                {
                    serial->writeValueJson(root, root->data.float64ArrayVal[i]);
                }
                serial->closeArray();
                break;
            case NodeType::eString:
                serial->writeValueJson(root, root->data.strVal, (root->len == 0) ? 0 : root->len - 1);
                break;
            case NodeType::eStringArray:
                serial->openArray();
                for (uint16_t i = 0; i < root->len; i++)
                {
                    serial->writeValueJson(root, root->data.strArrayVal[i]);
                }
                serial->closeArray();
                break;
            case NodeType::eObject:
    ...
    if ((root->flags & JsonNode::fFlagEnum) != 0)
    {
        ...
        serial->writeValueJson(root, true);
    }
