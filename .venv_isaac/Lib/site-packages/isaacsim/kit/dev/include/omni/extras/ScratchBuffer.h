...
public:
    ...
    /** Array accessor.
     *
     *  @note This may return nullptr if the buffer has not been allocated yet (size is 0)
     *        or if allocation failed. Always check for nullptr before dereferencing.
     *
     *  @returns The base address of the buffer, or nullptr if not allocated.
     *           This address will be valid until the next call to resize().
     *           It is the caller's responsibility to protect access to this buffer as needed.
     */
    T* data()
    {
