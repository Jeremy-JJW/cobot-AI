...
public:
    ...
    [[deprecated("Use carb.eventdispatcher with kGlobalEventXXX instead")]] carb::events::IEventStream* getEventStream() const
    {
        return m_stream.get();
    }

    std::pair<carb::RStringKey, uintptr_t> getEventKey() const
    {
        const static carb::RStringKey kWindow("window");
        return std::make_pair(kWindow, reinterpret_cast<uintptr_t>(this));
    }
...
private:
    ...
    carb::events::IEventStreamPtr m_stream; // adapter
    omni::kit::Window* m_handle = nullptr;
    carb::eventdispatcher::ObserverGuard m_updateSubId;
    std::string m_title;
