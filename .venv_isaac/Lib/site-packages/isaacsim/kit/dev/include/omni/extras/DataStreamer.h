...
public:
    DataStreamer() noexcept
    {
        ...
        if (m_events == nullptr)
        {
            OMNI_LOG_ERROR(OMNI_LOG_DEFAULT_CHANNEL, "unable to acquire IEvents");
            return;
        }
        ...
        {
            OMNI_LOG_ERROR(OMNI_LOG_DEFAULT_CHANNEL, "unable to acquire ITasking");
            return;
        }
        ...
        if (!m_eventStream)
        {
            OMNI_LOG_ERROR(OMNI_LOG_DEFAULT_CHANNEL, "unable to create an event stream");
            return;
        }
    ...
    void flush() noexcept
    {
        CARB_LOG_INFO(CARB_LOG_DEFAULT_CHANNEL, "waiting for all tasks to finish");
        m_tasks.wait();
        CARB_LOG_INFO(CARB_LOG_DEFAULT_CHANNEL, "all tasks have finished");
    }
...
public:
    ...
    DataListener(carb::events::IEventStreamPtr stream) noexcept
    {
        ...
        if (m_dict == nullptr)
        {
            OMNI_LOG_ERROR(OMNI_LOG_DEFAULT_CHANNEL, "failed to acquire IDictionary");
            return;
        }

        m_sub = carb::events::createSubscriptionToPop(
            stream.get(),
            [this](const carb::events::IEvent* e) {
                ...
                if (child == nullptr)
                {
                    OMNI_LOG_ERROR(OMNI_LOG_DEFAULT_CHANNEL, "the event had no /type field?");
                    return;
                }
                ...
                if (child == nullptr)
                {
                    OMNI_LOG_ERROR(OMNI_LOG_DEFAULT_CHANNEL, "the event had no /data field?");
                    return;
                }
                auto buffer = carb::dictionary::detail::getStringView(m_dict, child).value_or("");
                onDataReceived(buffer.c_str(), buffer.size(), type);
            },
            0, "DataListener");
    }
