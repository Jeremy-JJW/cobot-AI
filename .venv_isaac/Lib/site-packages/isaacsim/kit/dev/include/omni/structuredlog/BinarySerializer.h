...
public:
     ...
     */
    template <typename T>
    void track([[maybe_unused]] T* v, uint16_t len)
    {
        static_assert(std::is_arithmetic<T>::value, "this is only valid for primitive types");
     ...
     */
    void track([[maybe_unused]] const char* const* v, const uint16_t* stringLengths, uint16_t len)
    {
        m_counter = alignOffset<uint16_t>(m_counter);
        m_counter += sizeof(uint16_t);
     ...
     */
    template <typename T>
    void trackFixed([[maybe_unused]] T* v, uint16_t len)
    {
        static_assert(std::is_arithmetic<T>::value, "this is only valid for primitive types");
        m_counter = alignOffset<T>(m_counter);
        m_counter += sizeof(T) * len;
...
namespace
{
void ignoreBlobWriterValidationError(const char*) noexcept
{
}
} // namespace
...
public:
    ...
    /** Get the number of bytes written to the blob.
     *  @returns The number of bytes written to the blob.
     *
     *  @remarks This was added for testing and probably has no other use.
     */
    size_t getWritten()
    {
        return m_written;
