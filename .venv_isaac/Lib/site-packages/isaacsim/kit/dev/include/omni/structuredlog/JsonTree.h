...
struct JsonNode
{
    ...
    /** This specifies that a string property should be pseudonymized.
     *  When set, the value of this property should be hashed before transmission.
     *  This flag is only valid for @ref NodeType::eString and
     *  @ref NodeType::eStringArray types.
     */
    static constexpr Flag fFlagPseudonymize = 0x08;
            ...
            switch (fuzz)
            {
                ...
                case JsonTreeCompareFuzz::eNoConstOrder:
                {
                    ...
                    for (uint16_t i = 0, j = 0; i < a->len; i++, j++)
                    {
                        ...
                        while (i < a->len && (a->data.objVal[i].flags & JsonNode::fFlagConst) != 0)
                        {
                            i++;
                        }
                        foundA = i < a->len;
                        ...
                        // skip to the next non-constant member
                        while (j < b->len && (b->data.objVal[j].flags & JsonNode::fFlagConst) != 0)
                        {
                            j++;
                        }
                        foundB = j < b->len;
                        ...
                        if (foundA != foundB ||
                            (foundA && foundB && !compareJsonTrees(&a->data.objVal[i], &b->data.objVal[j])))
                            return false;
                    }

                    // Compare the constants.
                    // Since we verified above that all non-constant members are the same and that
                    // both structs have the same member count, we just need to verify that each
                    // constant in `a` has a unique match in `b`.
                    // The `hits` array is used to guarantee uniqueness, even though it should not
                    // be needed in valid JSON due to the unique key requirement.
                    for (uint16_t i = 0; i < a->len; i++)
                    {
                ...
                case JsonTreeCompareFuzz::eNoOrder:
                {
                    ...
                    // Double loop compare to check every element against every other element.
                    // The `hits` array is used to guarantee uniqueness, even though it should not
                    // be needed in valid JSON due to the unique key requirement.
                    for (uint16_t i = 0; i < a->len; i++)
                    {
                    ...
                    // Check that every element had a unique match.
                    // Due to the check that `a->len` == `b->len` above, this means all elements in
                    // both match.
                    for (uint16_t i = 0; i < a->len; i++)
                    {
                        if (!hits[i])
...
public:
     ...
     */
    template <typename T>
    void track([[maybe_unused]] T value)
    {
     ...
     */
    template <typename T>
    void track([[maybe_unused]] const T* value, uint16_t len)
    {
        static_assert(std::is_arithmetic<T>::value, "this is only valid for primitive types");
...
public:
    ...
    bool createObject(JsonNode* node, uint16_t propertyCount)
    {
        ...
        if (b == nullptr)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "allocator ran out of memory (node = '%s', requested %zu bytes)",
                           node->name, propertyCount * sizeof(*node->data.objVal));
            return false;
        }
    ...
    bool createObjectArray(JsonNode* node, uint16_t propertyCount, uint16_t len)
    {
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "allocator ran out of memory (node = '%s', requested %zu bytes)",
                           node->name, (len * (1 + propertyCount)) * sizeof(*node->data.objVal));
            return false;
        }
    ...
    bool setNode(JsonNode* node, const char* const* data, uint16_t len)
    {
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "allocator ran out of memory (node = '%s', requested %zu bytes)",
                           node->name, len * sizeof(*node->data.strArrayVal));
            return false;
        }
        ...
        for (uint16_t i = 0; i < len; i++)
        {
            ...
            else
            {
                ...
                {
                    CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "allocator ran out of memory (requested %zu bytes)", s);
    ...
    bool setNode(JsonNode* node, const char* str)
    {
        ...
        if (len > UINT16_MAX)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                           "string length exceeds 64KiB maximum (node = '%s', %zu characters, str = '%64s...')",
                           node->name, len, str);
            return false;
        }
    ...
    bool setName(JsonNode* node, const char* name, uint16_t nameLen)
    {
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                           "allocator ran out of memory (name = '%s', requested %" PRIu16 " bytes)", name, nameLen);
            return false;
        }
    ...
    static bool setFlags(JsonNode* node, JsonNode::Flag flags)
    {
        ...
        constexpr JsonNode::Flag validFlags =
            JsonNode::fFlagConst | JsonNode::fFlagFixedLength | JsonNode::fFlagEnum | JsonNode::fFlagPseudonymize;
        bool result = true;
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "unknown flags were used %02x (node = '%s')", flags & ~validFlags,
                           node->name);
            return false;
        }
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                           "attempted to set node to be both const and fixed length (node = '%s')", node->name);
        ...
        {
            CARB_LOG_ERROR(
                CARB_LOG_DEFAULT_CHANNEL, "a node cannot be both constant and an enum (node = '%s')", node->name);
        ...
        {
            CARB_LOG_ERROR(
                CARB_LOG_DEFAULT_CHANNEL, "an empty array can not be made into an enum (node = '%s')", node->name);
            return false;
        }
        ...
        switch (node->type)
        {
            ...
            case NodeType::eString:
                ...
                {
                    CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                                   "an enum type must be on a non-object array type (node = '%s')", node->name);
                    return false;
                }
                break;
        ...
        // check for invalid pseudonymize usage
        if ((flags & JsonNode::fFlagPseudonymize) != 0)
        {
            switch (node->type)
            {
                case NodeType::eString:
                case NodeType::eStringArray:
                    // Valid types for pseudonymization
                    break;

                default:
                    CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                                   "fFlagPseudonymize can only be set on string or string array types (node = '%s')",
                                   node->name);
                    return false;
            }
        }
        ...
        switch (node->type)
        {
            ...
            case NodeType::eFloat32:
                ...
                {
                    CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                                   "fixed length cannot be set on a scalar node (node = '%s')", node->name);
                    result = !failOnNonFatal;
                }
                break;
            ...
            case NodeType::eObject:
                ...
                {
                    CARB_LOG_ERROR(
                        CARB_LOG_DEFAULT_CHANNEL, "const is meaningless on an object node (node = '%s')", node->name);
                    result = !failOnNonFatal;
                }
            ...
            case NodeType::eObjectArray:
                ...
                {
                    CARB_LOG_ERROR(
                        CARB_LOG_DEFAULT_CHANNEL, "const is meaningless on an object array (node = '%s')", node->name);
                    result = !failOnNonFatal;
                }
                break;
...
private:
    ...
    bool setNode(JsonNode* node, const T* data, uint16_t len, T** dest, NodeType type)
    {
        ...
        *dest = static_cast<T*>(m_alloc->alloc(len * sizeof(**dest)));
        ...
        {
            CARB_LOG_ERROR(
                CARB_LOG_DEFAULT_CHANNEL, "allocator ran out of memory (requested %zu bytes)", len * sizeof(**dest));
            return false;
        }
