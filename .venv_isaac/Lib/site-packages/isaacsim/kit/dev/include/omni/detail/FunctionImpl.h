...
struct FunctionData
{
    ...
    //!      after a \c reset.
    template <typename Assigner, typename F>
    [[nodiscard]] Result bind_unsafe(Assigner, F&& f)
    {
...
struct FunctionAssignerImpl<FunctionTraits<TReturn(TArgs...)>,
                            TPointer,
                            std::enable_if_t<std::conjunction<is_invocable_r<TReturn, TPointer, TArgs...>,
                                                              std::disjunction<std::is_pointer<TPointer>,
                                                                               ...
                                                                               std::is_member_object_pointer<TPointer>>>::value>>
{
    struct type : TrivialFunctorCharacteristics
    {
        ...
        [[nodiscard]] static Result initialize(FunctionBuffer& buffer, TPointer fn) noexcept
        {
...
struct FunctionAssignerImpl<FunctionTraits<TReturn(TArgs...)>,
                            ...
                            std::enable_if_t<std::conjunction<
                                ...
                                std::negation<std::is_same<Functor, omni::function<TReturn(TArgs...)>>>,
                                // must be a class or union type (not a pointer or reference to a function)
                                std::disjunction<std::is_class<Functor>, std::is_union<Functor>>,
                                // must be callable
                                is_invocable_r<TReturn, Functor&, TArgs...>>::value>>
{
    struct InBuffer : TrivialFunctorCharacteristics
    {
        ...
        template <typename F>
        [[nodiscard]] static Result initialize(FunctionBuffer& buffer, F&& func)
        {
...
// complaining about uninitialized access might have a valid point.
    ...
    struct HeapAllocated
    {
        ...
        [[nodiscard]] static Result clone(FunctionBuffer* target, FunctionBuffer const* source)
        {
        ...
        template <typename UFunctor>
        [[nodiscard]] static Result initialize(FunctionBuffer& buffer, UFunctor&& func)
        {
    ...
    using type =
        std::conditional_t<
            std::conjunction<std::bool_constant<(sizeof(Functor) <= kFUNCTION_BUFFER_SIZE && kFUNCTION_BUFFER_ALIGN % alignof(Functor) == 0)>,
