 ...
 *  @param[in] timestampNs_ Parameter from schema at path '/timestampNs'.
 *             the relative timestamp in nanoseconds since an arbitrary epoch
 *             (usually system boot) when this event occurred.  This value should
 *             only be used to calculate the relative time differences between
 *             events.  Note that this timestamp value will not include any time
 *             that the system is suspended, sleeping, or hibernating.
 ...
 *           crashes.
 *
 *  @sa @ref Schema_omni_processlifetime_1_2::event_sendEvent().
 *  @sa @ref Schema_omni_processlifetime_1_2::event_isEnabled().
 */
#define OMNI_OMNI_PROCESSLIFETIME_1_2_EVENT(eventFlags_, event_, context_, timestampNs_)                               \
    OMNI_STRUCTURED_LOG(                                                                                               \
        omni::structuredlog::Schema_omni_processlifetime_1_2::event, eventFlags_, event_, context_, timestampNs_)
 ...
 *           panic, etc).
 *
 *  @sa @ref Schema_omni_processlifetime_1_2::heartbeat_sendEvent().
 *  @sa @ref Schema_omni_processlifetime_1_2::heartbeat_isEnabled().
 */
#define OMNI_OMNI_PROCESSLIFETIME_1_2_HEARTBEAT()                                                                      \
    OMNI_STRUCTURED_LOG(omni::structuredlog::Schema_omni_processlifetime_1_2::heartbeat)

class Schema_omni_processlifetime_1_2
{
public:
    ...
    enum : uint64_t
    {
        kEventEventId = OMNI_STRUCTURED_LOG_EVENT_ID(
            "omni.processlifetime", "com.nvidia.carbonite.processlifetime.event", "1.2", "0"),
        kHeartbeatEventId = OMNI_STRUCTURED_LOG_EVENT_ID(
            "omni.processlifetime", "com.nvidia.carbonite.processlifetime.heartbeat", "1.2", "0"),
    };

    Schema_omni_processlifetime_1_2() = default;
     ...
     *             extra information about the event that occurred.  This may be
     *             nullptr or an empty string if not needed for a particular event.
     *  @param[in] timestampNs Parameter from schema at path '/timestampNs'.
     *             the relative timestamp in nanoseconds since an arbitrary epoch
     *             (usually system boot) when this event occurred.  This value should
     *             only be used to calculate the relative time differences between
     *             events.  Note that this timestamp value will not include any time
    ...
    static void event_sendEvent(omni::structuredlog::IStructuredLog* strucLog,
                                ...
                                const omni::structuredlog::StringView& context,
                                uint64_t timestampNs) noexcept
    {
        _event_sendEvent(strucLog, eventFlags, event, context, timestampNs);
    }
    ...
    static bool _registerSchema(omni::structuredlog::IStructuredLog* strucLog)
    {
        ...
        if (strucLog == nullptr)
        {
            OMNI_LOG_WARN(OMNI_LOG_DEFAULT_CHANNEL,
                          ...
                          "'Schema_omni_processlifetime_1_2' "
                          "will be disabled.");
            return false;
        }
        ...
        buffer = strucLog->allocSchema("omni.processlifetime", "1.2", flags, bufferSize, &handle);
        if (buffer == nullptr)
        {
...
#    pragma warning(pop)
    ...
    static size_t _event_calculateTreeSize()
    {
        ...
        calc.trackObject(3); // object has 3 properties
        {
            ...
            // property timestampNs
            calc.trackName("timestampNs");
            calc.track(uint64_t(0));
        }
        return calc.getSize();
    }
    ...
    static omni::structuredlog::JsonNode* _event_buildJsonTree(size_t bufferSize, uint8_t* buffer)
    {
        ...
        *base = {};
        ...
        }
        {
            ...
            // property timestampNs
            result = builder.setName(&base->data.objVal[2], "timestampNs");
            if (!result)
            {
                OMNI_LOG_ERROR(OMNI_LOG_DEFAULT_CHANNEL, "failed to set the object name (bad size calculation?)");
                return nullptr;
            }
            result = builder.setNode(&base->data.objVal[2], uint64_t(0));
            if (!result)
            {
                OMNI_LOG_ERROR(OMNI_LOG_DEFAULT_CHANNEL, "failed to set type 'uint64_t' (shouldn't be possible)");
                return nullptr;
            }
        }
...
OMNI_STRUCTURED_LOG_ADD_SCHEMA(omni::structuredlog::Schema_omni_processlifetime_1_2, omni_processlifetime, 1_2, 0);
