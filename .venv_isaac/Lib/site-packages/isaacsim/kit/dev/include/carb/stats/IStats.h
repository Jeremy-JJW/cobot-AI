...
struct IStats
{
    CARB_PLUGIN_INTERFACE("carb::stats::IStats", 2, 2)
     ...
     * Use getOrCreateNestedStat() instead for the ability to add nested stats.
     *
    ...
    /**
     * Retrieves the entire stats added for the specified scopeId, including the numbers of the added stats, that are
     * directly under the scope and not children of other states. Call enumerateChildrenStatNodes() recursively on each
     * returned StatNode.statId, to get their children nodes. It is thread safe.
     *
     * Note: This API will also update the elapsed timer value that is used by stats that specified kScopeFlagRate.
     *
     ...
     *                   if nodeCount is only needed.
     * @param nodeCount  The numbers of the added nodes without children. It can be nullptr.
     */
    ...
    */
    ...
    /**
     * Inserts a new stat within a scope or returns an existing one if it is already added.
     * Unlike getOrCreateStat(), allows nested stats to be inserted to a parent stat node.
     *
     * @param statDesc A StatDesc describing the desired stat. Must be fully filled out.
     * @returns The ID of the new stat object, or nullptr in case of a failure.
     *
     ...
     * and it is thread safe.
     */
    StatId(CARB_ABI* getOrCreateNestedStat)(const NestedStatDesc& statDesc);

    /**
     * Retrieves the entire stats added directly as children of the specified statId, including the numbers of the
     * children stats. When statId is set to nullptr, all root nodes are returned (similar to enumerateStatNodes()).
     * It is thread safe.
     *
     * @param scopeId    The scope identifier that created the statId.
     * @param statId     The parent stat identifier to get the children stat nodes from. If statId is set to nullptr,
     *                   all root nodes are returned like enumerateStatNodes().
     * @param nodes      A pointer to an array of StatNode to get all the info of children stats. It can be nullptr,
     *                   if nodeCount is only needed.
     * @param nodeCount  The numbers of the added nodes as children of the statId. It can be nullptr.
     */
    void(CARB_ABI* enumerateChildrenStatNodes)(ScopeId scopeId, StatId statId, StatNode* nodes, size_t* nodeCount);

    /**
     * Returns the total number of stat nodes for the specified scope, including all the nested children.
     * This can be used to pre-allocate the size required for the entire nested calls to enumerateChildrenStatNodes().
     */
    uint64_t(CARB_ABI* getTotalStatNodeCount)(ScopeId scopeId);
};
