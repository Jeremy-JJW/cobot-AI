...
#define carb_Framework_latest CARB_HEXVERSION(0, 8)
#ifndef carb_Framework
//! The Carbonite Framework default version
#    define carb_Framework CARB_HEXVERSION(0, 7)
#endif
...
//! great thought.
#define CARBONITE_MAJOR (carb_Framework >> 16)
...
#define CARBONITE_MINOR (carb_Framework & 0xffff)
...
//!
//! Called when a plugin is registered that exports the requested interface. Note that the plugin has not yet been
//! started.
//!
//! The thread that registers the plugin will call all register hooks for the requested interface. It is safe to acquire
//! the requested interface from within the register hook. Calling register hooks is done without an internal Framework
//! mutex locked, so other threads are able to acquire other interfaces while register hooks are executing.
//!
//! It is safe for a `RegisterHookFn` to call \ref Framework::removeRegisterHook() for the `RegisterHookFn` being
//! called, or any other `RegisterHookFn`.
//!
//! @see Framework::addRegisterHook() Framework::removeRegisterHook()
//! @param plugin The \ref PluginDesc for the plugin that was registered.
//! @param userData The `void*` that was passed to \ref Framework::addRegisterHook().
using RegisterHookFn = void (*)(const PluginDesc& plugin, void* userData);
...
//! Releases the Carbonite framework immediately and exits the process, without running C/C++
//! atexit() registered functions or static destructors.
//!
//! This shutdown and terminate function should only be called in cases where it is known that
//! any pending user data has already been saved and the intention is a quick and clean exit
//! from the process.
//!
...
//!
//! All registered plugins will be shutdown and unloaded and any registered C/C++ atexit()
//! functions will be run.  This shutdown and terminate function should only be called in
//! cases where the intention is to perform a full shutdown of all plugins and interfaces
//! in a way that guarantees the process will exit.
//!
//! @note This function does not return.
//!
//! @warning This function must not be called from within a DLL, shared object, or plugin.
//!
//! This function performs the following sequence:
//! 1. Calls any exported @ref carbOnPluginShutdown functions on all loaded plugins if the
//!    framework is acquired.  Plugins will be unloaded, unregistered and all interfaces
//!    will be released.
//! 2. All built-in interfaces are shutdown and released.
//! 3. Calls any registered Framework release hooks (see @ref carb::Framework::addReleaseHook())
//!    in reverse order of registration if the framework is acquired.
//! 4. Flushes stdout/stderr.
//! 5. Calls `TerminateProcess()` on Windows or `_exit()` on Linux.
//!
//! @thread_safety May be called from any thread.
//! @param[in] exitCode The exit code that the process will exit with.
CARB_DYNAMICLINK void releaseFrameworkAndTerminate [[noreturn]] (int exitCode);

//! Unloads all plugins and shuts down the Carbonite Framework without terminating the process.
//!
//! Releases the Carbonite framework immediately and shuts down all registered plugins and
//! interfaces properly.  The process is not terminated upon return.  That exercise is left
//! up to the caller.  This shutdown function should only be called in cases where the intention
//! is to shutdown the framework and release as many resources as possible before returning.
//! Upon return, no Carbonite framework or interface functions may be called.  It is not
//! guaranteed that the Carbonite framework can be restarted within the same process after
...
//!
//! @thread_safety May be called from any thread.
CARB_DYNAMICLINK void releaseFrameworkAndShutdown();
...
struct Framework
{
    ...
    //! Acquires all interfaces of the given type, starting any plugins that export the interface type in question.
    //!
    ...
    //!     recommended. Instead, use `omni::vector<T*> acquireInterfaces()`
    //! @endrst
    template <typename T>
#if CARB_VERSION_ATLEAST(carb_Framework, 0, 8)
    [[deprecated("Use acquireInterfaces() that returns an omni::vector")]]
#endif
    void
    acquireInterfaces(T** interfaces, uint32_t interfacesSize);
    ...
    //! use \ref removeLoadHook(). It is possible to add multiple load hooks with the same parameters, but this is
    ...
    template <class T>
    [[nodiscard]] LoadHookHandle addLoadHook(const char* pluginName, LoadHookFn func, void* userData);
...
#if CARB_VERSION_ATLEAST(carb_Framework, 0, 6)
    //! @private
    bool(CARB_ABI* internalAddReleaseHook2)(const char* client, void* iface, ReleaseHookFn fn, void* user);
#endif

#if CARB_VERSION_ATLEAST(carb_Framework, 0, 7)
    //! @private
    RegisterHookHandle(CARB_ABI* internalAddRegisterHook)(
        const InterfaceDesc& iface, const char* plugin, const char* clientName, RegisterHookFn func, void* userData);

    //! Adds a plugin registration hook that is called when a plugin is registered that exports the desired interface.
    //!
    //! This can be used as asynchronous notification when an interface becomes available due to a plugin being
    //! registered that exports that interface. To remove the register hook, use \ref removeRegisterHook(). It is
    //! possible to add multiple register hooks with the same parameters, but this is not recommended and will cause
    //! @p func to be called multiple times with the same parameters.
    //!
    //! @note Plugins that export the given interface may already be registered. @p func will not be called for these
    //!   interfaces, only plugins that are registered after calling this function.
    //!
    //! See \ref RegisterHookFn for more information on how and when register hooks are called.
    //!
    //! @see RegisterHookFn removeRegisterHook()
    //! @tparam T The interface type. \b NOTE: The interface version is not checked, only the name. It is possible for
    //!   @p func to be invoked notifying of an interface that cannot be acquired due to version mismatch.
    //! @param pluginName an optional plugin name desired that exposes \c T, or \c nullptr for any plugin.
    //! @param func the \ref RegisterHookFn to call when a plugin that exports the requested interface is registered.
    //!   This function may be called multiple times if multiple plugins are registered that export the requested
    //!   interface. This function may be called from multiple threads simultaneously.
    //! @param userData application-specific data that is supplied to \p func when it is called.
    //! @returns A \ref RegisterHookHandle uniquely identifying this register hook, or \ref kInvalidRegisterHook if an
    //!   error occurs or the Framework has already been shut down. Call \ref removeRegisterHook() to remove the hook.
    template <typename T>
    [[nodiscard]] RegisterHookHandle addRegisterHook(const char* pluginName, RegisterHookFn func, void* userData);

    //! Removes a previously-added register hook.
    //!
    //! It is safe to remove the register hook from within the register hook callback.
    //!
    //! @param handle The \ref RegisterHookHandle returned from \ref addRegisterHook().
    //! @returns \c true if the register hook was found and removed; \c false otherwise.
    bool(CARB_ABI* removeRegisterHook)(RegisterHookHandle handle);
#endif

#if CARB_VERSION_ATLEAST(carb_Framework, 0, 8)
    //! @private
    void(CARB_ABI* internalAcquireInterfaces)(const char* clientName,
                                              InterfaceDesc interfaceDesc,
                                              bool existing,
                                              void (*callback)(void* iface, void* ud),
                                              void* userData);

    /**
     * Acquires all interfaces of a given type, starting any plugins that export the interface type in question.
     * @tparam The interface type to acquire
     * @returns A \ref omni::vector of interfaces
     */
    template <class T>
    [[nodiscard]] omni::vector<T*> acquireInterfaces();

    /**
     * Acquires all interfaces of a given type, without starting any plugins.
     * @tparam The interface type to acquire
     * @returns A \ref omni::vector of interfaces
     */
    template <class T>
    [[nodiscard]] omni::vector<T*> acquireExistingInterfaces();
#endif
};
} // namespace carb
...
//! plugins.
CARB_WEAKLINK CARB_HIDDEN carb::cpp::zstring_view g_carbClientName;
...
//!                         lifetime of the module.  Must not be empty.
//! @returns `true`.
//!
//! @note This should not be called directly.  This is called as part of CARB_FRAMEWORK_GLOBALS().
inline bool setClientName(cpp::unbounded_string clientName)
{
    g_carbClientName = cpp::zstring_view(cpp::unsafe_length, clientName);
    return !g_carbClientName.empty();
}
//! @copydoc setClientName
inline bool setClientName(cpp::zstring_view clientName)
{
...
{
    auto clientName = g_needToCall_CARB_GLOBALS_atGlobalScope ? g_carbClientName.c_str() : nullptr;
...
inline bool Framework::addReleaseHook(void* iface, ReleaseHookFn fn, void* user)
{
    auto clientName = g_needToCall_CARB_GLOBALS_atGlobalScope ? g_carbClientName.c_str() : nullptr;
#if CARB_VERSION_ATLEAST(carb_Framework, 0, 6)
    if (this->internalAddReleaseHook2) // Version was not bumped when this was added, so still need null check
...
template <typename T>
[[nodiscard]] LoadHookHandle Framework::addLoadHook(const char* pluginName, LoadHookFn func, void* user)
{
    auto clientName = g_needToCall_CARB_GLOBALS_atGlobalScope ? g_carbClientName.c_str() : nullptr;
...
#if CARB_VERSION_ATLEAST(carb_Framework, 0, 7)
template <typename T>
[[nodiscard]] RegisterHookHandle Framework::addRegisterHook(const char* pluginName, RegisterHookFn func, void* userData)
{
    auto clientName = g_needToCall_CARB_GLOBALS_atGlobalScope ? g_carbClientName.c_str() : nullptr;
    return this->internalAddRegisterHook(T::getInterfaceDesc(), pluginName, clientName, func, userData);
}
#endif

#if CARB_VERSION_ATLEAST(carb_Framework, 0, 8)
template <class T>
omni::vector<T*> Framework::acquireInterfaces()
{
    omni::vector<T*> ret;
    auto clientName = g_needToCall_CARB_GLOBALS_atGlobalScope ? g_carbClientName.c_str() : nullptr;
    this->internalAcquireInterfaces(
        clientName, T::getInterfaceDesc(), false,
        [](void* iface, void* ud) { static_cast<omni::vector<T*>*>(ud)->push_back(static_cast<T*>(iface)); }, &ret);
    return ret;
}

template <class T>
omni::vector<T*> Framework::acquireExistingInterfaces()
{
    omni::vector<T*> ret;
    auto clientName = g_needToCall_CARB_GLOBALS_atGlobalScope ? g_carbClientName.c_str() : nullptr;
    this->internalAcquireInterfaces(
        clientName, T::getInterfaceDesc(), true,
        [](void* iface, void* ud) { static_cast<omni::vector<T*>*>(ud)->push_back(static_cast<T*>(iface)); }, &ret);
    return ret;
}
#endif
