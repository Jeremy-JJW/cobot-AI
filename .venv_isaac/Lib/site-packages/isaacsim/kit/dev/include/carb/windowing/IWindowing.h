...
#    define carb_windowing_IWindowing CARB_HEXVERSION(1, 5)
#endif
...
struct IWindowing
{
     ...
     *
     * Any previous thread-local error is cleared on entry. If the requested change cannot be
     * performed (e.g. no monitor or video mode is available when entering fullscreen), the
     * function returns without modifying the window and sets the thread's current error via
     * @ref carb::ErrorApi. Callers can check @ref carb::ErrorApi::getError() after this call
     * to detect failure; carb::ErrorCode::kFail indicates the monitor or video mode was
     * unavailable.
     *
     * @param window The window to use.
