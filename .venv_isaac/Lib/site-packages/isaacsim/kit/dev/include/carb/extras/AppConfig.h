...
public:
    ...
    {
        static constexpr cpp::string_view kSpecialRawStringMarker("$raw:");
        ...
        auto sv = dictionary::detail::getStringView(dictInterface, item);
        if (!sv)
        {
            return nullptr;
        }

        if (!sv->starts_with(kSpecialRawStringMarker))
        {
            return nullptr;
        }

        return sv->c_str() + kSpecialRawStringMarker.size();
    }
                                                     ...
                                                     const EnvVariablesMap* envVariables)
    {
        ...
        const auto itemResolver = [&](dictionary::Item* item, uint32_t, void*) -> uint32_t {
            ...
            auto stringView = dictionary::detail::getStringView(dictInterface, item).value_or("");
            ...
            auto [resolvedString, valid] =
                extras::resolveEnvVarReferences(stringView.c_str(), stringView.length(), *envVariables);

            if (!valid)
            {
                CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                               "Error while resolving environment variable references for '%.*s'",
                               (int)stringView.length(), stringView.c_str());
                return 0;
            }

            if (!resolvedString.empty())
            {
                dictionary::detail::setString(dictInterface, item, resolvedString);
            }
            else
            {
        ...
        const auto getChildByIndexMutable = [](dictionary::IDictionary* dict, dictionary::Item* item, size_t index) {
            return dictionary::detail::getItemChildByIndex(dict, item, index);
        };
                                                          ...
                                                          void* userData)
    {
        ...
        if (updaterData->getItemFullPathFunc)
        {
            ...
            CARB_LOG_VERBOSE(CARB_LOG_DEFAULT_CHANNEL,
                             "Replacing the '%s' item current value by the value from '%s' config.", itemPath.c_str(),
                                                              ...
                                                              const EnvVariablesMap* envVariablesMap)
    {
        ...
        UpdaterData updaterData = { dictInterface, newDictSource ? newDictSource : "Unspecified source",
                                    getFullPathFunc() };
        dictInterface->update(targetDict, {}, newDict, {}, onDictUpdateReporting, &updaterData);
                                            ...
                                            const char* configSubFolderName = "nvidia")
    {
        ...
        static constexpr char kUserFolderEnvVar[] = "USERPROFILE";
#elif CARB_PLATFORM_LINUX
        static constexpr char kUserFolderEnvVar[] = "XDG_CONFIG_HOME";
#endif
                                                       ...
                                                       const EnvVariablesMap* envVariablesMap)
    {
        if (pathwiseEnvOverridesMap)
        {
            ...
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Couldn't process environment variables overrides");
        }
                                            ...
                                            const std::string& elementValue)
    {
        ...
        dictionary::Item* parsedDictionary = jsonSerializerInterface->createDictionaryFromString(elementValue);
        if (!parsedDictionary)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Couldn't parse as JSON data command-line argument '%s'",
                           elementPath.c_str());
            return;
        }
        ...
        dictionaryInterface->update(targetDictionary, elementPath, parsedDictionary, {},
                                                   ...
                                                   const EnvVariablesMap* envVariablesMap)
    {
        if (cmdLineOptionsMap)
        {
                ...
                for (const auto& kv : *cmdLineOptionsMap)
                {
                    ...
                    if (trimmedValue.size() >= 2)
                    {
                        ...
                        else if (trimmedValue.front() == '{' && trimmedValue.back() == '}')
                        {
                            ...
                            {
                                CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                                               "No JSON serializer acquired. Cannot process command line parameter '%s'",
                                               kv.first.c_str());
                            }
                            continue;
                        }
            ...
            {
                CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Couldn't process command line overrides");
            }
        }
        return combinedConfig;
    }
