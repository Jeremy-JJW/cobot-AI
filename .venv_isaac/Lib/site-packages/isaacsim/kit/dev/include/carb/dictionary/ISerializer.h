...
struct ISerializer
{
     ...
     *                    correctly for the implementation, the operation will fail.
     *  @param[in] len The length of the string data, if known. If not known, provide `size_t(-1)`
     *                 as the length, which is also the default.
     ...
     *           vice versa), if the string is malformed, or has a syntax error in it.
     */
    dictionary::Item* createDictionaryFromString(carb::cpp::string_view string,
                                                 DeserializerOptions options = kDeserializerOptionNone)
    {
        return internalCreateDictionaryFromStringBuffer(string.data(), string.size(), options);
    }
};
