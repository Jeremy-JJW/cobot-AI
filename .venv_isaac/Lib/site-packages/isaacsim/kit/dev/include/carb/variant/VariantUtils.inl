...
inline VariantData traits::copy(const VariantData& self) noexcept
{
    ...
    {
        return self.vtable->Copy(&self); // Note: the returned type can be different
    }
...
inline bool traits::convertTo(const VariantData& self, const VTable* newType, VariantData& out) noexcept
{
    if (self.vtable && newType)
    {
        ...
        {
            // Note: the output type can be different than requested.
            return self.vtable->ConvertTo(&self, newType, &out);
        }
    }
    return false;
}

inline bool traits::convertFrom(const VTable* desired, VariantData& out, const VariantData& from) noexcept
{
    if (!desired || !from.vtable)
        return false;
    if (from.vtable == desired)
        return (out = traits::copy(from), true);
    if (!CARB_INCLUDES_MEMBER(desired->sizeOf, VTable::ConvertFrom))
        return false;
    return desired->ConvertFrom ? desired->ConvertFrom(desired, &out, &from) : false;
}
...
cpp::optional<T> Variant::getValue() const noexcept
{
    ...
    auto desired = iface->getVTable(t.type());
    if (traits::convertTo(data(), desired, temp) || traits::convertFrom(desired, temp, data()))
    {
        CARB_SCOPE_EXIT
        {
...
Variant Variant::convertTo() const noexcept
{
    ...
    CARB_ASSERT(iface, "Failed to acquire interface IVariant");
    auto desired = iface->getVTable(t.type());
    if (traits::convertTo(data(), desired, out) || traits::convertFrom(desired, out, data()))
    {
        return out;
    }
    out.vtable = nullptr; // prevent destruction since out is in undefined state
    return {};
}
