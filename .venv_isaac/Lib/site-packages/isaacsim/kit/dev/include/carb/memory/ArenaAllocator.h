...
{
    using FallbackAllocTraits = std::allocator_traits<FallbackAllocator>;

public:
    //! \c T*
    using pointer = typename FallbackAllocTraits::pointer;
    //! `T const*`
    using const_pointer = typename FallbackAllocTraits::const_pointer;
    //! \c void*
    using void_pointer = typename FallbackAllocTraits::void_pointer;
    //! `void const*`
    using const_void_pointer = typename FallbackAllocTraits::const_void_pointer;
    //! \c T
    using value_type = typename FallbackAllocTraits::value_type;
    //! \c std::size_t
    using size_type = typename FallbackAllocTraits::size_type;
    //! \c std::ptrdiff_t
    using difference_type = typename FallbackAllocTraits::difference_type;
    ...
    struct rebind
    {
        ...
        using other = ArenaAllocator<U, typename FallbackAllocTraits::template rebind_alloc<U>>;
    };
...
private:
    ...
    template <class M, class N, class Al1, class Al2>
    friend bool operator==(const ArenaAllocator<M, Al1>&, const ArenaAllocator<N, Al2>&);
};
