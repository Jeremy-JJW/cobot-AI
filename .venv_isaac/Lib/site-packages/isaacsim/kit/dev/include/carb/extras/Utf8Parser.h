...
//! The official Unicode REPLACEMENT CHARACTER, U+FFFD
constexpr CodePoint kReplacementChar{ 0xFFFD };

//! The maximum possible Unicode code point
constexpr CodePoint kLastCodePoint{ 0x10FFFF };

// UTF-16 surrogate pairs
constexpr char16_t kSurrogateFirstMask{ 0xf800 }; //!< Mask for a UTF-16 surrogate pair first code unit
constexpr char16_t kSurrogateFirst{ 0xd800 }; //!< Value for a UTF-16 surrogate pair first code unit
constexpr char16_t kSurrogateSecondMask{ 0xfc00 }; //!< Mask for a UTF-16 surrogate pair second code unit
constexpr char16_t kSurrogateSecond{ 0xdc00 }; //!< Value for a UTF-16 surrogate pair second code unit
constexpr char16_t kSurrogateLast{ 0xdfff }; //!< Highest possible UTF-16 surrogate pair code unit value
constexpr char32_t kSurrogateBias{ 0x10000 }; //!< The starting value of a code point represented by a surrogate pair
constexpr size_t kSurrogateShift = 10; //!< The number of encoding bits used in a surrogate pair code unit
//! Mask of encoding bits in a surrogate pair code unit
...
{
/**
 * A std::string-like adapter class for using a buffer.
 */
template <typename CharT>
class BufferAdapter
{
public:
    using value_type = CharT; //!< Value type.

    /**
     * Constructor.
     * @param buffer Pointer to the buffer to fill. May be `nullptr` to measure.
     * @param bufferLen The number of `CharT`s in @p buffer (NOT bytes).
     */
    ...
    /**
     * Resizes the number of characters in the buffer and fills with empty `CharT`s.
     * @param size The new size. Must be greater than the previous size.
     * @returns \c true if the buffer supports the new size. \c false otherwise. Note that these return values
     *   differ from the standard std::string implementation.
     */
    ...
    /**
     * Access operator.
     * @param index The index to access. Must be less than size().
     * @returns a reference to the index, or a dummy location if no buffer was provided.
     */
    ...
    /**
     * Capacity accessor.
     * @returns The capacity (in `CharT`s) of the buffer adapter.
     */
    ...
    /**
     * Size accessor.
     * @returns The size of the string (in `CharT`s) of the buffer adapter.
     */
     ...
     * @returns \c true if the character was added; \c false if the buffer does not have space. Note that returning a
     *   value is a departure from the `std::string` implementation.
     */
    ...
    /**
     * Access size() but including the terminator.
     * @returns The size of the string in the buffer including the null terminator. If no buffer was provided, this will
     *   return the size needed to hold the string including the null terminator. If a zero-size valid buffer was
     *   provided, this will return 0.
     */
...
//! Returns whether the given code point is valid.
//! @param cp The code point to check.
//! @returns \c false if @p cp is invalid: beyond the last code point or in the surrogate pair range.
...
//! Ensures that a code point is valid by replacing invalid code points with the replacement character.
//! @param cp The code point to check.
//! @returns \p cp if it was valid, otherwise the replacement character `U+FFFD`.
...
//! Adapter function for calling non-standard functions in BufferAdapter or standard std::string functions.
//! @param pred The predicate to call.
//! @returns If @p pred returns a `bool` value, this function will return that value. Otherwise always returns `true`.
...
/**
 * Helper to perform a conversion using a string.
 * @tparam StringType the string type to return.
 * @tparam In The character type of the input string.
 * @param[in] str The input string to convert.
 * @param[out] out The @c StringType that will receive the output. May be a \c BufferAdapter.
 * @returns \c true if the string was converted without truncation; \c false if the output was truncated.
 */
 ...
 * @tparam StringType The output string type, must have a \c value_type member type that is \c char32_t.
 * @param[in] str The input UTF-8 string to convert.
...
//! @copydoc convertUtf8StringToUtf32(cpp::string_view)
 ...
 * @param[in]  str    The input UTF-32 string to convert.
...
/**
 * Convert a UTF-32 encoded string to UTF-8.
 * @tparam StringType The output string type, must have a \c value_type member type that is \c char or \c char8_t.
 * @param[in]  str The input UTF-32 string to convert.
...
//! @copydoc convertUtf32StringToUtf8(cpp::u32string_view)
 ...
 * @param[in]  str    The input UTF-16 string to convert.
 * @param[out] out    The output buffer to hold the UTF-8 data.
 *                    This must be at least @p outLen bytes in length.
 *                    This can be `nullptr` to calculate the required output buffer length.
 *                    The output string written to @p out will always be null terminated
...
//! @copydoc convertUtf16StringToUtf8(cpp::u16string_view)
...
/**
 * Convert a UTF-8 encoded string to UTF-16.
 * @tparam StringType The output string type, must have a \c value_type member type that is \c char16_t.
 * @param[in]  str The input UTF-8 string to convert.
...
//! @copydoc convertUtf8StringToUtf16(cpp::string_view)
 ...
 *       wide strings. Please use UTF-8, UTF-16 or UTF-32 for new systems.
 */
...
/**
 * Convert a UTF-8 encoded string to a wide string.
 * @tparam StringType The output string type, must have a \c value_type member type that is \c wchar_t.
 * @param[in]  str The input UTF-8 string to convert.
 * @returns @p str converted to wide.
 * @note This is provided for interoperability with older systems that still use wide strings. Please use UTF-8, UTF-16
 *   or UTF-32 for new systems.
 */
template <class StringType>
...
//! @copydoc convertUtf8StringToWide(cpp::string_view)
 ...
 * @param[in]  str    The input wide string to convert to UTF-8.
 * @param[out] out    The output buffer to hold the wide data.
 *                    This must be at least @p outLen in length, in elements.
 *                    This can be `nullptr` to calculate the required output buffer length.
 *                    The output string written to @p out will always be null terminated
 ...
 * @tparam StringType The output string type, must have a \c value_type member type that is \c char or \c char8_t.
 * @param[in]  str The input wide string to convert.
 * @returns @p str converted to UTF-8.
 *
 * @note This is provided for interoperability with older systems that still use wide strings. Please use UTF-8, UTF-16
 *   or UTF-32 for new systems.
...
/**
 * Convert a wide char string (UTF-16 or UTF-32) to UTF-8.
 * @param[in]  str The input wide string to convert. This may not be `nullptr`.
 * @returns @p str converted to UTF-8.
 * @note This is provided for interoperability with older systems that still use wide strings. Please use UTF-8, UTF-16
 *   or UTF-32 for new systems.
 */
...
//! @copydoc convertWideStringToUtf8(cpp::wstring_view)
