...
public:
     ...
     */
    explicit RHUnorderedMultiset(size_type reserved,
     ...
     * @param al Allocator to use for all memory allocations of this container.
     */
    RHUnorderedMultiset(size_type reserved, const allocator_type& al)
        : RHUnorderedMultiset(reserved, hasher(), key_equal(), al)
    {
    }
     ...
     * @param al Allocator to use for all memory allocations of this container.
     */
    RHUnorderedMultiset(size_type reserved, const hasher& h, const allocator_type& al)
        : RHUnorderedMultiset(reserved, h, key_equal(), al)
    {
    }
     ...
     * @param al Allocator to use for all memory allocations of this container.
     */
    explicit RHUnorderedMultiset(const allocator_type& al) : Base(al)
    {
    }
    ...
    RHUnorderedMultiset(InputIt first,
                        InputIt last,
                        size_type reserved = 0,
                        const hasher& h = hasher(),
                        const key_equal& ke = key_equal(),
                        const allocator_type& al = allocator_type())
        : RHUnorderedMultiset(reserved, h, ke, al)
    {
     ...
     */
    template <class InputIt>
    RHUnorderedMultiset(InputIt first, InputIt last, size_type reserved, const allocator_type& al)
        : RHUnorderedMultiset(reserved, hasher(), key_equal(), al)
    {
        // Since first and last are input iterators, we only get one iteration through the list and cannot measure it.
        while (first != last)
     ...
     */
    template <class InputIt>
    RHUnorderedMultiset(InputIt first, InputIt last, size_type reserved, const hasher& h, const allocator_type& al)
        : RHUnorderedMultiset(reserved, h, key_equal(), al)
    {
        // Since first and last are input iterators, we only get one iteration through the list and cannot measure it.
        while (first != last)
     ...
     * @param al Allocator to use for all memory allocations of this container.
     */
    RHUnorderedMultiset(std::initializer_list<value_type> init,
                        size_type reserved = 0,
                        const hasher& h = hasher(),
                        const key_equal& ke = key_equal(),
                        const allocator_type& al = allocator_type())
        : RHUnorderedMultiset(reserved ? reserved : init.size(), h, ke, al)
    {
        for (auto& val : init)
            insert(val);
    }
     ...
     * @param al Allocator to use for all memory allocations of this container.
     */
    RHUnorderedMultiset(std::initializer_list<value_type> init, size_type reserved, const allocator_type& al)
        : RHUnorderedMultiset(reserved ? reserved : init.size(), hasher(), key_equal(), al)
    {
        for (auto& val : init)
            insert(val);
    }
    ...
    RHUnorderedMultiset(std::initializer_list<value_type> init,
        ...
        : RHUnorderedMultiset(reserved ? reserved : init.size(), h, key_equal(), al)
    {
        for (auto& val : init)
            insert(val);
    }
     ...
     */
    RHUnorderedMultiset(const RHUnorderedMultiset& other, const Allocator& alloc) : Base(other, alloc)
    {
    }
     ...
     * @param alloc The Allocator to use for all memory allocations of this container.
     */
    RHUnorderedMultiset(RHUnorderedMultiset&& other, const Allocator& alloc) : Base(std::move(other), alloc)
    {
    }
    ...
    /**
     * Erases elements.
     * \details Removes specified elements from the container. All elements with the key that compares equivalent to the
     * given value @p key are removed.
     *
     * References, pointers and iterators to the erase element are invalidated. All other iterators, pointers and
     * references remain valid.
    ...
    /**
     * Returns the number of elements matching the specified key.
     * \details Returns the number of elements with key that compares equivalent to the specified argument @p key.
     * @note This overload participates in overload resolution only if Hash and KeyEqual are both transparent. This
     * assumes that such Hash is callable with both `K` and `Key` type and that KeyEqual is transparent, which,
     * together, allows calling this function without constructing an instance of Key.
...
RHUnorderedMultiset(InputIt, InputIt, size_t = 0, Hash = Hash(), Pred = Pred(), Alloc = Alloc())
    -> RHUnorderedMultiset<detail::ValueFromIt_t<InputIt>, Hash, Pred, LoadFactorMax100, Alloc>;
...
RHUnorderedMultiset(std::initializer_list<T>, size_t = 0, Hash = Hash(), Pred = Pred(), Alloc = Alloc())
    -> RHUnorderedMultiset<T, Hash, Pred, LoadFactorMax100, Alloc>;

template <typename InputIt, typename Alloc>
RHUnorderedMultiset(InputIt, InputIt, size_t, Alloc)
    -> RHUnorderedMultiset<detail::ValueFromIt_t<InputIt>,
...
RHUnorderedMultiset(InputIt, InputIt, size_t, Hash, Alloc)
    -> RHUnorderedMultiset<detail::ValueFromIt_t<InputIt>, Hash, std::equal_to<detail::ValueFromIt_t<InputIt>>, kDefaultLoadFactor, Alloc>;

template <typename T, typename Alloc>
RHUnorderedMultiset(std::initializer_list<T>, size_t, Alloc)
    -> RHUnorderedMultiset<T, std::hash<T>, std::equal_to<T>, kDefaultLoadFactor, Alloc>;

template <typename T, typename Hash, typename Alloc>
RHUnorderedMultiset(std::initializer_list<T>, size_t, Hash, Alloc)
    -> RHUnorderedMultiset<T, Hash, std::equal_to<T>, kDefaultLoadFactor, Alloc>;
#endif
