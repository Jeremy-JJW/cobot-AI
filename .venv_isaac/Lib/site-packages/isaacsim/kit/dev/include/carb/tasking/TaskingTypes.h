...
/**
 * Thread Index type.
 * \details Returned by \ref ITasking::getCurrentThreadIndex() to determine what type of thread a task is running on.
 * If called from a task running on a worker thread, this will return a numeric index, otherwise it returns one of the
 * enumerations indicating the type of thread.
 *
 * The numeric thread index will be in the range `[0, max)` where `max` is \ref TaskingDesc::threadCount as returned by
 * \ref ITasking::getDesc().
 *
 * If called from a thread not owned by *carb.tasking*, kExternal will be returned.
 */
enum class ThreadIndex : uint32_t
{
    //! Indicates an emergency thread. Note: while there may be multiple emergency threads, they are not indexed. Use
    //! the thread ID to differentiate between them.
    kEmergency = uint32_t(-3),

    //! Indicates the main thread. Only one thread may be known as the Main thread for the lifetime of *carb.tasking*.
    kMain = uint32_t(-2),

    //! Indicates an external thread. Note: while there may be multiple external threads, they are not indexed. Use the
    //! thread ID to differentiate between them.
    kExternal = uint32_t(-1),
};
