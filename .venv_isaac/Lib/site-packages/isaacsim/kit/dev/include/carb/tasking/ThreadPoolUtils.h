...
struct ApplyWithPromise
{
    ...
    {
        promise.set_value(std::forward<ReturnType>(std::apply(std::forward<Callable>(f), std::forward<Tuple>(t))));
    }
};
...
struct ApplyWithPromise<void>
{
    ...
    {
        std::apply(std::forward<Callable>(f), std::forward<Tuple>(t));
        promise.set_value();
    }
};
...
public:
    ...
    ThreadPoolWrapper(IThreadPool* poolInterface, size_t workerCount = 0) : m_interface(poolInterface)
    {
        ...
        {
            CARB_LOG_ERROR(
                CARB_LOG_DEFAULT_CHANNEL, "IThreadPool interface used to create a thread pool wrapper is null.");
            return;
        }
        ...
        if (m_pool == nullptr)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Couldn't create a new thread pool.");
        }
    }
    ...
    size_t getWorkerCount() const
    {
        if (!isValid())
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                           "Attempt to call the 'getWorkerCount' method of an invalid thread pool wrapper.");
            return 0;
        }
    ...
    auto enqueueJob(Callable&& task, Args&&... args)
    {
        ...
        {
            CARB_LOG_ERROR(
                CARB_LOG_DEFAULT_CHANNEL, "Attempt to call the 'enqueueJob' method of an invalid thread pool wrapper.");
            return Future{};
        }
        ...
        if (!pData)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "ThreadPoolWrapper: No memory for job");
            return Future{};
        }
        ...
        CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "ThreadPoolWrapper: failed to enqueue job");
        delete pData;
        return Future{};
    }
    ...
    size_t getCurrentlyRunningJobCount() const
    {
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                           "Attempt to call the 'getCurrentlyRunningJobCount' method of an invalid thread pool wrapper.");
            return 0;
        }
    ...
    void waitUntilFinished() const
    {
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                           "Attempt to call the 'waitUntilFinished' method of an invalid thread pool wrapper.");
            return;
        }
