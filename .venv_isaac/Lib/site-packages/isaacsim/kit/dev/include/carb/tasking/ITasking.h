...
#define carb_tasking_ITasking_latest CARB_HEXVERSION(2, 7)
#ifndef carb_tasking_ITasking
//! Default version of ITasking interface to use
#    define carb_tasking_ITasking CARB_HEXVERSION(2, 6)
#endif
...
struct ITasking
{
...
#if CARB_VERSION_ATLEAST(carb_tasking_ITasking, 2, 6)
    /**
     * Returns the thread index of the thread the function is called from.
     *
     * @see ThreadIndex
     * @returns A \ref ThreadIndex indicating the index or type of the current thread.
     */
    ThreadIndex(CARB_ABI* getCurrentThreadIndex)();
#endif

#if CARB_VERSION_ATLEAST(carb_tasking_ITasking, 2, 7)
    /**
     * Waits on a ConditionVariable object until it is notified or the timeout period expires.
     *
     * The given @p m must match the same @p m passed in by all other threads/tasks waiting on the ConditionVariable,
     * and must be locked by the current thread/task. Before waiting, the @p unlock function is called. When the
     * thread/task is notified the @p lock function is called before returning to the caller. ConditionVariables are
     * allowed to spuriously wake up, so best practice is to check the condition in a loop and wait again if the
     * condition does not match desired.
     *
     * @note It is recommended to use \ref ConditionVariableAnyWrapper.
     * @param cv The ConditionVariable to wait on.
     * @param unlock The function to call to unlock the mutex (given by @p m)
     * @param lock The function to call to lock the mutex (given by @p m)
     * @param m The generic mutex that is locked by the current thread/task.
     * @param timeoutNs The relative timeout period in nanoseconds. Specify kInfinite to wait forever or 0 to test
     * immediately without waiting.
     * @returns true if the condition variable was notified; false if the timeout period expired.
     */
    bool(CARB_ABI* timedWaitConditionVariableAny)(
        ConditionVariable* cv, void (*unlock)(void*), void (*lock)(void*), void* m, uint64_t timeoutNs);
#endif
    ...
    {
        [[maybe_unused]] bool b = internalFutexWait(&val, &compare, sizeof(T), kInfinite);
        CARB_ASSERT(b);
    }
