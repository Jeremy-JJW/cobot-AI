...
public:
     ...
     * @note If multiple elements in the range `[first, last)` have equivalent keys, the first encountered key will be
     * inserted.
     ...
     * @note If multiple elements in @p init have equivalent keys, the first encountered key will be inserted.
     ...
     * Inserts an element into the container.
     *
     * If \c *this already contains an element which transparent compares \a equivalent to @p value, does nothing.
     * Otherwise, constructs an object u of \c value_type with `std::forward<K>(value)` and then inserts u into `*this`.
     *
     * If `equal_range(u) != hash_function()(obj) || contains(u)` is `true`, behavior is undefined. The \c value_type
     * must be \a EmplaceConstructible into \c *this from `std::forward<K>(value)`.
     *
     * If insertion is successful, all iterators, references and pointers are invalidated.
     ...
     * @returns Number of elements with key that compares equivalent to @p key.
     */
    template <typename K, typename = std::enable_if_t<IsTransparent_v<K>>>
