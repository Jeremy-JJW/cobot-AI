                                                            ...
                                                            const std::map<std::string, std::string>& envVariables) noexcept
{
    ...
    if (!sourceBuffer)
    {
        CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Null source buffer for resolving environment variable references.");
        return result;
    }
    ...
    static constexpr char kEnvVarPrefix[] = "$env{";
    constexpr const size_t kEnvVarPrefixLength = carb::countOf(kEnvVarPrefix) - 1;
    static constexpr char kEnvVarPostfix = '}';
         ...
         curReferencePos = std::strstr(endEnvPos, kEnvVarPrefix))
    {
        ...
        if (!endEnvPos)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                           "Couldn't find the end of the environment variable reference: '%s'", curReferencePos);
            return result;
        }
    ...
    auto envVarResolver = [&](const char* buffer, const char* bufferEnd, size_t firstRefIndexToTry,
        ...
        for (; curBufPos < bufferEnd && curRefIndex < envReferencesSize; ++curRefIndex)
        {
            ...
            {
                CARB_LOG_WARN(CARB_LOG_DEFAULT_CHANNEL, "Found environment variable reference with empty name.");
            }
    ...
    static constexpr char kElvisOperator[] = "?:";
