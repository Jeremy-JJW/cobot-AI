...
#else
            throw std::runtime_error(carb::fmt::format("Failed to acquire interface: {} (pluginName: {})",
...
InterfaceType* getCachedInterfaceForBindings()
{
    ...
    CARB_UNLIKELY_IF(!iface)
    {
        ...
        throw std::runtime_error(
            carb::fmt::format("Failed to acquire cached interface: {}", InterfaceType::getInterfaceDesc().name));
#endif
    }
    return iface;
}
...
#else
        throw std::runtime_error(carb::fmt::format(
...
inline Framework* acquireFrameworkForBindings(const char* scriptLanguage)
{
    ...
    Framework* f = acquireFramework(g_carbClientName.c_str());
    if (!f)
        return nullptr;
    g_carbFramework = f;

    // Register as binding for the given script language
    f->registerScriptBinding(BindingType::Binding, g_carbClientName.c_str(), scriptLanguage);
