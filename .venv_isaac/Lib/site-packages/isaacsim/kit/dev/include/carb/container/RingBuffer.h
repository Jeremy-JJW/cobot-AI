...
public:
    uint8_t* Allocate(size_t& size)
    {
        ...
        for (;;)
        {
            ...
            [[maybe_unused]] DWORD err1 = ::GetLastError();
            ...
            [[maybe_unused]] DWORD err2 = ::GetLastError();
...
{
    CARB_UNUSED(padding1, padding2); // gcc11 doesn't like `[[maybe_unused]]` specified on members
...
inline void RingBuffer::commit(void* mem)
{
    ...
    [[maybe_unused]] uint32_t result = header->bytes.fetch_or(Header::kCommitted, std::memory_order_release);
...
{
    return this->read([&obj](void* mem, [[maybe_unused]] size_t size) {
        using Type = typename std::decay<T>::type;
