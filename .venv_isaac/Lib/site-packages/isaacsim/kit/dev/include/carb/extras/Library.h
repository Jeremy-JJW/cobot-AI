...
#    define CARB_LIBRARY_EXTENSION_WIDE L".dll"
/** UTF-16 version of the dynamic library extension. */
#    define CARB_EXECUTABLE_EXTENSION_WIDE L".exe"
 ...
 *                          included and will also be present in the created filename.
 ...
 */
inline std::string createLibraryNameForModule(carb::cpp::string_view baseName)
{
    if (baseName.empty())
        return {};
#if CARB_PLATFORM_WINDOWS
    ...
    size_t pos = baseName.find_last_of('/');
#endif
    size_t pathLen = pos == carb::cpp::string_view::npos ? 0 : pos + 1;
    return join(baseName.substr(0, pathLen), CARB_LIBRARY_PREFIX, baseName.substr(pathLen), CARB_LIBRARY_EXTENSION);
}
...
{
    std::string fullLibName; // do not move this variable from here, it is used to keep the string alive until the end
                             // of the function
    ...
    {
        fullLibName = createLibraryNameForModule(cpp::string_view(cpp::unsafe_length, libraryName));
    ...
    std::wstring widecharName = convertCarboniteToWindowsPath(libraryName);
    ...
    {
        [[maybe_unused]] HMODULE h = nullptr;
        [[maybe_unused]] BOOL b = GetModuleHandleExW(CARBWIN_GET_MODULE_HANDLE_EX_FLAG_PIN, widecharName.c_str(), &h);
        CARB_ASSERT(b != 0);
...
#    endif
    ...
    if (handle == nullptr && libraryName != nullptr && libraryName[0] != 0)
    {
        ...
        if (path == nullptr)
        {
            ...
            CARB_LOG_VERBOSE(CARB_LOG_DEFAULT_CHANNEL, "realpath(%s) failed (errno = %d)", libraryName, errno);
            return nullptr;
        }
    ...
    if (handle != nullptr)
    {
        ...
        if (dlinfo(handle, RTLD_DI_LINKMAP, &linkMap) == -1 || linkMap == nullptr)
        {
            CARB_LOG_WARN(CARB_LOG_DEFAULT_CHANNEL,
                          "Library '%s' loaded with errors '%s' and no link map.  The likely cause of this is that ",
                          libraryName, errorMsg);
            CARB_LOG_WARN(CARB_LOG_DEFAULT_CHANNEL,
                          "a dependent library or symbol in the dependency chain is missing.  Use the environment ");
            CARB_LOG_WARN(CARB_LOG_DEFAULT_CHANNEL, "variable 'LD_DEBUG=all' to diagnose.");
...
{
#if CARB_PLATFORM_WINDOWS
    return getLastWinApiErrorMessage();
#else
    return dlerror();
#endif
}
...
inline void unloadLibrary(LibraryHandle libraryHandle)
{
    if (libraryHandle)
    {
        ...
        if (!::FreeLibrary(libraryHandle))
        {
            ...
            CARB_LOG_WARN(CARB_LOG_DEFAULT_CHANNEL, "FreeLibrary for handle %p failed with error: %" PRIdword "/%s",
                          libraryHandle, err, convertWinApiErrorCodeToMessage(err).c_str());
        }
#elif CARB_POSIX
        if (::dlclose(libraryHandle) != 0)
        {
            CARB_LOG_WARN(
                CARB_LOG_DEFAULT_CHANNEL, "Closing library handle %p failed with error: %s", libraryHandle, dlerror());
        }
#else
        CARB_UNSUPPORTED_PLATFORM();
#endif
...
inline std::string getLibraryFilenameByHandle(LibraryHandle handle)
{
    ...
    for (;;)
    {
        ...
        if (res == 0)
        {
            // CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "GetModuleFileNameW(%p) failed (%d)", handle, GetLastError());
            return "";
        }
        if (res < path.size())
        {
        ...
        CARB_FATAL_UNLESS(suc, "failed to allocate %zu bytes", path.size() * 2);
    }

    return convertWindowsToCarbonitePath(path.data());
#elif CARB_PLATFORM_LINUX
    struct link_map* map;
    ...
    if (res != 0)
    {
        // CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "failed to retrieve the link map from library handle %p (%d)",
        // handle, errno);
        return "";
    }
    ...
    if (!map->l_name || map->l_name[0] == '\0')
    {
        ...
        {
            // CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "library had no filename in the link map but was not the main
            // module");
            return {};
        }
...
{
    return getPathParent(getLibraryFilenameByHandle(handle)).getString();
}
...
{
    return getPathParent(getLibraryFilename(symbolAddress)).getString();
}
...
struct FreeLib
{
    void operator()(LibraryHandle lib) noexcept
    {
        unloadLibrary(lib);
    }
};
 ...
 *  This will close the library handle when it goes out of scope.
 */
using UniqueLibrary = std::unique_ptr<std::remove_pointer_t<LibraryHandle>, detail::FreeLib>;
