...
constexpr const char* getGlobalSemaphoreName()
{
...
#        endif
    ...
    ~NamedSemaphore()
    {
        [[maybe_unused]] int result = sem_close(m_sema);
...
#        if CARB_PLATFORM_LINUX
        auto printMessage = [](const char* format, ...) {
            ...
            {
                CARB_LOG_WARN(CARB_LOG_DEFAULT_CHANNEL, "%s", buffer);
            }
...
public:
    ...
    bool open(const OpenToken& openToken, AccessMode access = AccessMode::eDefault)
    {
        ...
        if (token == nullptr)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "failed to allocate memory for the open token for this SHM region.");
            return false;
        }
...
#if CARB_PLATFORM_WINDOWS
        std::wstring fname = carb::extras::convertUtf8ToWide(mappingName);
        ...
        if (handle.handleWin32 == nullptr)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                           "failed to open a file mapping object with the name '%s' {error = %" PRIdword "}",
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "failed to create the reference count object with the name '%s'.",
                           token->name);
            free(token);
            return false;
        }
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                           "failed to open or create file mapping object with the name '%s' {errno = %d/%s}",
    ...
    protected:
         ...
         */
        bool map(SharedHandle handle, size_t offset, size_t size, AccessMode access, [[maybe_unused]] size_t allocGran)
        {
            void* mapPtr = nullptr;
            ...
            {
                CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                               "failed to map %zu bytes from offset %zu {error = %" PRIdword "}", size, offset,
                               GetLastError());
                return false;
            }
#elif CARB_POSIX
            ...
            {
                CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "failed to map %zu bytes from offset %zu {errno = %d/%s}",
                               size, offset, errno, strerror(errno));
                return false;
            }
#else
            CARB_UNSUPPORTED_PLATFORM();
        ...
        void unmap()
        {
            ...
            if (UnmapViewOfFile(m_address) == CARBWIN_FALSE)
                CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "failed to unmap the region at %p {error = %" PRIdword "}",
                               m_address, GetLastError());
#elif CARB_POSIX
            if (munmap(m_address, m_size) == -1)
                CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "failed to unmap the region at %p {errno = %d/%s}", m_address,
     ...
     */
    void close([[maybe_unused]] bool forceUnlink = false)
    {
        if (m_token == nullptr)
            return;
...
#pragma pack(pop)
    ...
    std::string getPlatformMappingName(const char* name, size_t maxLength = 0)
    {
        std::string fname;
#if CARB_PLATFORM_WINDOWS
        static constexpr char prefix[] = "Local\\";
        ...
        CARB_UNSUPPORTED_PLATFORM();
#endif
        fname = carb::extras::join(carb::cpp::unsafe_length, prefix, name);
    ...
    std::string makeUniqueName(const char* name)
    {
        ...
        return carb::extras::join(carb::cpp::unsafe_length, name, carb::cpp::string_view(buffer, len));
    }
    ...
    Result createAndOrOpen(const char* name, size_t size, uint32_t flags, bool tryOpen, bool tryCreate)
    {
        ...
        if (token == nullptr)
        {
            if (!quiet)
                CARB_LOG_ERROR(
                    CARB_LOG_DEFAULT_CHANNEL, "failed to create a new open token for the SHM region '%s'.", name);
            return eError;
        }
            ...
            if (pNtQuerySection)
            {
                ...
                if (pNtQuerySection(handle.handleWin32, 0 /*SectionBasicInformation*/, &sbi, sizeof(sbi), &read) >= 0)
                {
                    if (size > (size_t)sbi.MaximumSize.QuadPart)
                    {
                        if (!quiet)
                            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                                           "mapping with name '%s' was opened but existing size %" PRId64
        ...
        if (handle.handleFd == -1)
        {
            ...
            {
                if (!quiet)
                    CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "failed to create/open SHM region '%s' {errno = %d/%s}",
            ...
            {
                if (!quiet)
                    CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "failed to stat SHM region '%s' {errno = %d, %s}", name,
                                   errno, strerror(errno));
            ...
            if (size > size_t(statbuf.st_size) && ftruncate(handle.handleFd, (off64_t)size) != 0)
            {
                if (!quiet)
                    CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                                   "failed to grow the size of the SHM region '%s' from %zu to %zu bytes {errno = %d/%s}",
        ...
        else if (ftruncate(handle.handleFd, (off64_t)size) != 0)
        {
            if (!quiet)
                CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                               "failed to set the size of the SHM region '%s' to %zu bytes {errno = %d/%s}", name, size,
                               errno, strerror(errno));
            ::close(handle.handleFd);
            shm_unlink(mappingName.c_str());
            destroyRefCount(token->name);
    ...
    bool initRefCount(const char* name, int flags, bool logError)
    {
        ...
        if (m_refCount == SEM_FAILED)
        {
            ...
            {
                CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL,
                               "failed to create or open a semaphore named \"%s\" {errno = %d/%s}", mappingName.c_str(),
                               errno, strerror(errno));
            }
            return false;
        }
