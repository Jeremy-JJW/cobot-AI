...
/** Version tag to mark ABI compatibility. */
...
public:
     ...
     *                          events::IEvents, so you should memcpy the
...
protected:
    ...
    void onEventReceived(const carb::events::IEvent* e) noexcept override
    {
        ...
        auto getIntVal = [this](const carb::dictionary::Item* root, const char* name) -> size_t {
            ...
            return (child == nullptr) ? 0 : (size_t)m_dict->getAsInt64(child);
        };
        switch (e->type)
        {
 ...
 *  This will send a stream of audio data through events::IEvents then
