...
public:
    ...
    protected:
        ...
        void assertOwner(const IntrusiveUnorderedMultimap*) const noexcept
        {
        }
        void assertSameOwner(const const_iterator&) const noexcept
        {
        }
        void assertNotEnd() const noexcept {}
    ...
    void rehash(size_t buckets)
    {
        if (buckets > m_bucketCount)
        {
            ...
            Hash hasher;
            for (Link* next; cur != _end(); cur = next)
            {
                next = cur->m_prev;
                ...
                else
                {
                    ...
                    cur->m_prev = _end();
                    cur->m_next = _end()->m_next; // _head(), but the GCC 11+ optimizer doesn't handle it properly
                    cur->m_prev->m_next = cur;
                    cur->m_next->m_prev = cur;
