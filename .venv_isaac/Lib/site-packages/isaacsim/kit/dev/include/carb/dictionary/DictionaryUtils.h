 ...
 *
 *  @param[in] dict     The @ref IDictionary interface to use to access the items in the dictionary.  This must not be
 *                      `nullptr`.  This must be the same interface that was originally used to create the dictionary @p
 *                      baseItem.
 *  @param[in] baseItem The base item to retrieve the item name relative to.  This is expected to contain the child path
 *                      @p path.  This may not be `nullptr`.
 *  @param[in] path     The item path relative to @p baseItem that indicates where to find the item whose name should be
 *                      retrieved.  This may be an empty string to retrieve the name of @p baseItem itself.
 *  @returns A string containing the name of the item at the given path relative to @p baseItem if it exists.  Returns
 *           an empty string if no item could be found at the requested path or a string buffer could not be allocated
 *           for its name.
 *
 *  @thread_safety This call is thread safe.
 */
inline omni::string getStringFromItemName(const IDictionary* dict, const Item* baseItem, carb::cpp::string_view path = {})
{
    ...
    return dict->createStringFromItemName(item).value_or(omni::string{});
}
 ...
 *                      `nullptr`.  This must be the same interface that was originally used to create the dictionary @p
 *                      baseItem.
 *  @param[in] baseItem The base item to retrieve the item value relative to.  This is expected to contain the child
 *                      path @p path.  This may not be `nullptr`.
 *  @param[in] path     The item path relative to @p baseItem that indicates where to find the item whose value should
 *                      be retrieved.  This may be an empty string to retrieve the value of @p baseItem itself.
 *  @returns A string containing the value of the item at the given path relative to @p baseItem if it exists.  If the
 *           requested item was not of type @ref ItemType::eString, the value will be converted to a string as best it
 *           can. Returns an empty string if no item could be found at the requested path or a string buffer could not
 *           be allocated for its name.
 *
 *  @thread_safety This call is thread safe.
 */
inline omni::string getStringFromItemValue(const IDictionary* dict, const Item* baseItem, carb::cpp::string_view path = {})
{
    const Item* item = dict->getItem(baseItem, path);
    if (!item)
    {
    ...
    return dict->createStringFromItemValue(item).value_or(omni::string{});
}
 ...
 *                      `nullptr`.  This must be the same interface that was originally used to create the dictionary @p
 *                      baseItem.
 *  @param[in] baseItem The base item to retrieve the item values relative to.  This is expected to contain the child
 *                      path @p path.  This may not be `nullptr`.
 *  @param[in] path     The item path relative to @p baseItem that indicates where to find the item whose value should
 *                      be retrieved.  This may be an empty string to retrieve the values of @p baseItem itself.  The
 *                      value at this path is expected to be an array of strings.
 *  @returns A vector of string values for the array at the path @p path relative to @p baseItem. If the given path is
 *           not an array item, a vector containing a single value will be returned.  If @p path points to an item that
 *           is an array of something other than strings, a vector of empty strings will be returned instead.
 *
 *  @thread_safety This call in itself is thread safe, however the retrieved array may contain unexpected or incorrect
 *                 values if another thread is modifying the same item in the dictionary simultaneously.
 */
inline std::vector<std::string> getStringArray(const IDictionary* dict,
                                               const Item* baseItem,
                                               carb::cpp::string_view path = {})
{
    ...
    if (!itemAtKey)
        return {};
    ScopedRead lock(*dict, itemAtKey);
    std::vector<std::string> stringArray(dict->getArrayLength(itemAtKey));
    for (size_t i = 0; i < stringArray.size(); i++)
    {
        stringArray[i] = std::string{ dict->getStringViewAt(itemAtKey, i).value_or("") };
    }
    return stringArray;
}
 ...
 *
 *  @param[in] dict        The @ref IDictionary interface to use to access the items in the dictionary.  This must not
 *                         be `nullptr`.  This must be the same interface that was originally used to create the
 *                         dictionary @p baseItem.
 *  @param[in] baseItem    The base item to act as the root of where to set the values relative to.  This is expected
 *                         to contain the child path @p path.  This may not be `nullptr`.
 *  @param[in] path        The path to the item to set the array of strings in.  This path must either already exist as
 *                         an array of strings or be an empty item in the dictionary.  This may be `nullptr` or an empty
 *                         string to set the string array into the item @p baseItem itself.
 *  @param[in] stringArray The array of strings to set in the dictionary.  This may contain a different number of items
 *                         than the existing array.  If the number of items differs, this new array of values will
 *                         replace the existing item at @p path entirely.  If the count is the same as the previous
 *                         item, values will simply be replaced.
 *  @returns No return value.
 *
 *  @thread_safety This call itself is thread safe as long as no other call is trying to concurrently modify the same
 *                 item in the dictionary.  Results are undefined if another thread is modifying the same item in the
 *                 dictionary. Similarly, undefined behavior may result if another thread is concurrently trying to
 *                 retrieve the items from this same dictionary.
 */
template <class ContainerOfSVLikes>
void setStringArray(IDictionary* dict, Item* baseItem, carb::cpp::string_view path, const ContainerOfSVLikes& stringArray)
{
    ScopedWrite lock(*dict, baseItem);
    Item* itemAtKey = dict->getItem(baseItem, path);
    if (itemAtKey)
        dict->destroyItem(itemAtKey);
    itemAtKey = dict->makeDictionaryAtPath(baseItem, path);
    ...
    dict->setItemFlag(itemAtKey, dictionary::ItemFlag::eUnitSubtree, true);
    for (size_t i = 0, stringCount = stringArray.size(); i < stringCount; ++i)
    {
        dict->setStringAt(itemAtKey, i, stringArray[i]);
    }
}
 ...
 *  @thread_safety This call is thread safe.
 */
inline void setDictionaryElementAutoType(IDictionary* id,
                                         Item* dict,
                                         carb::cpp::string_view path,
                                         carb::cpp::string_view value)
{
    if (!path.empty())
    {
        ...
        if (value.empty())
        {
            id->makeStringAtPath(dict, path, value);
            return;
        }
            ...
            ((value.front() == '"' && value.back() == '"') || (value.front() == '\'' && value.back() == '\'')))
        {
            ...
            id->makeStringAtPath(dict, path, value.substr(1, value.size() - 2));
            return;
        }

        // Convert the value to upper case to simplify checks
        omni::string uppercaseValue(value);
        std::transform(value.begin(), value.end(), uppercaseValue.begin(),
                       [](const char c) { return static_cast<char>(carb::toupper(static_cast<unsigned char>(c))); });

        // let's see if it's a boolean
        if (uppercaseValue == cpp::string_view("TRUE"))
        {
            id->makeBoolAtPath(dict, path, true);
            return;
        }
        if (uppercaseValue == cpp::string_view("FALSE"))
        {
            id->makeBoolAtPath(dict, path, false);
            return;
        }
        ...
        const long long int valueAsInt = strtoll(uppercaseValue.c_str(), &endptr, 0);

        if (endptr - uppercaseValue.c_str() == ptrdiff_t(uppercaseValue.size()))
        {
            id->makeInt64AtPath(dict, path, valueAsInt);
            return;
        }

        // let's see if it's a float
        const double valueAsFloat = strtod(uppercaseValue.c_str(), &endptr);
        if (endptr - uppercaseValue.c_str() == ptrdiff_t(uppercaseValue.size()))
        {
            id->makeFloat64AtPath(dict, path, valueAsFloat);
            return;
        }

        // consider the value to be a string
        id->makeStringAtPath(dict, path, value);
    }
}
 ...
 */
template <class MappingOfSVLikes>
inline void setDictionaryFromStringMapping(IDictionary* id, Item* dict, MappingOfSVLikes&& mapping)
{
...
inline void setDictionaryArrayElementFromStringValue(dictionary::IDictionary* dictionaryInterface,
                                                     ...
                                                     carb::cpp::string_view elementPath,
                                                     carb::cpp::string_view elementValue)
{
    ...
    dictionary::Item* arrayItem = dictionaryInterface->getItem(targetDictionary, elementPath);
    if (arrayItem)
    {
    ...
    arrayItem = dictionaryInterface->makeDictionaryAtPath(targetDictionary, elementPath);
    // Setting necessary flag to make it a proper empty array
    // This will result in correct item replacement in case of dictionary merging
    ...
    const std::string commonElementPath = carb::extras::join(elementPath, "/");
    size_t curElementIndex = 0;

    // Helper adds provided value into the dictionary and increases index for the next addition
    auto dictElementAddHelper = [&](carb::cpp::string_view value) {
        ...
        if (value.empty())
        {
            CARB_LOG_WARN(
                ...
                (int)elementPath.size(), elementPath.data(), (int)elementValue.size(), elementValue.data());
            return;
        }
    ...
    auto lastValue = elementValue.substr(curValueStartPos, elementValue.size() - curValueStartPos - 1);
...
namespace detail
{
inline cpp::optional<cpp::zstring_view> getItemName(const IDictionary* dict, const Item* item)
{
    return dict->getItemName(item);
}

inline cpp::optional<cpp::zstring_view> getStringView(const IDictionary* dict, const Item* item)
{
    return dict->getStringView(item);
}

inline Item* createItem(IDictionary* dict, Item* baseItem, cpp::string_view path, ItemType type)
{
    return dict->createItem(baseItem, path, type);
}

inline const Item* getItem(const IDictionary* dict, const Item* base, cpp::string_view path)
{
    return dict->getItem(base, path);
}

inline Item* getItem(IDictionary* dict, Item* base, cpp::string_view path)
{
    return dict->getItem(base, path);
}

inline Item* getItemChildByIndex(IDictionary* dict, Item* base, size_t index)
{
    return dict->getItemChildByIndex(base, index);
}

inline void setString(IDictionary* dict, Item* item, cpp::string_view value)
{
    dict->setString(item, value);
}

template <class T>
inline Item* makeAtPath(IDictionary* dict, Item* base, cpp::string_view path, T value)
{
    return dict->makeAtPath(base, path, value);
}

inline Item* makeDictionaryAtPath(IDictionary* dict, Item* base, cpp::string_view path)
{
    return dict->makeDictionaryAtPath(base, path);
}

inline void setStringAt(IDictionary* dict, Item* item, size_t index, cpp::string_view value)
{
    dict->setStringAt(item, index, value);
}
} // namespace detail
//! \endcond
#else // ^ CARB_VERSION_ATLEAST(carb_dictionary_IDictionary, 2, 0)
static_assert(CARB_VERSION_ATLEAST(carb_dictionary_IDictionary, 1, 2),
              "carb::dictionary::IDictionary 1.2 required as a minimum");
 ...
 *  @thread_safety This call is thread safe.
 */
inline std::string getStringFromItemNameS(const IDictionary* dict, const Item* baseItem, carb::cpp::string_view path = {})
{
    const Item* item = dict->getItemS(baseItem, path);
    if (!item)
    {
        return {};
    }
 ...
 *  @thread_safety This call is thread safe.
 */
inline std::string getStringFromItemValueS(const IDictionary* dict, const Item* baseItem, carb::cpp::string_view path = {})
{
    const Item* item = dict->getItemS(baseItem, path);
    if (!item)
    {
 ...
 *                 values if another thread is modifying the same item in the dictionary simultaneously.
 */
inline std::vector<std::string> getStringArrayS(const IDictionary* dict,
                                                const Item* baseItem,
                                                carb::cpp::string_view path = {})
{
    const Item* itemAtKey = dict->getItemS(baseItem, path);
    if (!itemAtKey)
        return {};
    ScopedRead lock(*dict, itemAtKey);
    std::vector<std::string> stringArray(dict->getArrayLength(itemAtKey));
    for (size_t i = 0; i < stringArray.size(); i++)
    {
        stringArray[i] = std::string{ dict->getStringBufferAtS(itemAtKey, i).value_or("") };
    }
    return stringArray;
}
 ...
 */
template <class ContainerOfSVLikes>
inline void setStringArrayS(IDictionary* dict,
                            Item* baseItem,
                            carb::cpp::string_view path,
                            const ContainerOfSVLikes& stringArray)
{
    ScopedWrite lock(*dict, baseItem);
    Item* itemAtKey = dict->getItemMutableS(baseItem, path);
    if (itemAtKey)
        dict->destroyItem(itemAtKey);
    itemAtKey = dict->makeDictionaryAtPathS(baseItem, path);
    // Setting necessary flag to make it a proper empty array
    // This will result in correct item replacement in case of dictionary merging
    dict->setItemFlag(itemAtKey, dictionary::ItemFlag::eUnitSubtree, true);
    for (size_t i = 0, stringCount = stringArray.size(); i < stringCount; ++i)
    {
        dict->setStringAtS(itemAtKey, i, stringArray[i]);
    }
}
...
inline cpp::optional<cpp::zstring_view> getItemName(const IDictionary* dict, const Item* item)
{
    return dict->getItemNameS(item);
}

inline cpp::optional<cpp::zstring_view> getStringView(const IDictionary* dict, const Item* item)
{
    return dict->getStringBufferS(item);
}

inline Item* createItem(IDictionary* dict, Item* baseItem, cpp::string_view path, ItemType type)
{
    return dict->createItemS(baseItem, path, type);
}

inline const Item* getItem(const IDictionary* dict, const Item* base, cpp::string_view path)
{
    return dict->getItemS(base, path);
}

inline Item* getItem(IDictionary* dict, Item* base, cpp::string_view path)
{
    return dict->getItemMutableS(base, path);
}

inline Item* getItemChildByIndex(IDictionary* dict, Item* base, size_t index)
{
    return dict->getItemChildByIndexMutable(base, index);
}

inline void setString(IDictionary* dict, Item* item, cpp::string_view value)
{
    dict->setStringS(item, value);
}

template <class T>
inline Item* makeAtPath(IDictionary* dict, Item* base, cpp::string_view path, T value)
{
    // if constexpr (std::is_convertible_v<T, const char*>) // Dictionary 1.2 still supports const char*
    //    return dict->makeAtPath(base, path.c_str(), value);
    // else
    return dict->makeAtPathS(base, path, value);
}

inline Item* makeDictionaryAtPath(IDictionary* dict, Item* base, cpp::string_view path)
{
    return dict->makeDictionaryAtPathS(base, path);
}

inline void setStringAt(IDictionary* dict, Item* item, size_t index, cpp::string_view value)
{
    dict->setStringAtS(item, index, value);
}
} // namespace detail
 ...
 *                      @p path.  This may not be `nullptr`.
 *  @param[in] path     The item path relative to @p baseItem that indicates where to find the item whose name should be
 *                      retrieved.  This may be `nullptr` or an empty string to retrieve the name of @p baseItem itself.
 *  @returns A string containing the name of the item at the given path relative to @p baseItem if it exists.  Returns
 *           an empty string if no item could be found at the requested path or a string buffer could not be allocated
 *           for its name.
 *
...
{
    // old functions need to stick to the old API (this will be deprecated shortly)
    const Item* item = dict->getItem(baseItem, path);
    if (!item)
    {
        return {};
 ...
 *
 *  @param[in] dict     The @ref IDictionary interface to use to access the items in the dictionary.  This must not be
 *                      `nullptr`.  This must be the same interface that was originally used to create the dictionary
 *                      @p baseItem.
 *  @param[in] baseItem The base item to retrieve the item value relative to.  This is expected to contain the child
 *                      path @p path.  This may not be `nullptr`.
 *  @param[in] path     The item path relative to @p baseItem that indicates where to find the item whose value should
 *                      be retrieved.  This may be `nullptr` or an empty string to retrieve the value of @p baseItem
 *                      itself.
 *  @returns A string containing the value of the item at the given path relative to @p baseItem if it exists.  If the
 *           requested item was not of type @ref ItemType::eString, the value will be converted to a string as best it
 *           can. Returns an empty string if no item could be found at the requested path or a string buffer could not
 ...
 *                      path @p path.  This may not be `nullptr`.
 *  @param[in] path     The item path relative to @p baseItem that indicates where to find the item whose value should
 *                      be retrieved.  This may be `nullptr` or an empty string to retrieve the values of @p baseItem
 ...
 *                  nullptr`.  This must be the same interface that was originally used to create the dictionary @p
 *                  baseItem.
 *  @param[in] item The base item to retrieve the item values relative to.  This is expected to contain the child path
 *                  @p path.  This may not be `nullptr`.
 *  @returns A vector of string values for the array at the path @p path relative to @p baseItem. If the given path is
 *           not an array item, a vector containing a single value will be returned.  If @p path points to an item that
 *           is an array of something other than strings, a vector of empty strings will be returned instead.
 ...
 *                         be `nullptr`.  This must be the same interface that was originally used to create the
 *                         dictionary @p baseItem.
 *  @param[in] baseItem    The base item to act as the root of where to set the values relative to.  This is expected to
 *                         contain the child path @p path.  This may not be `nullptr`.
 *  @param[in] path        The path to the item to set the array of strings in.  This path must either already exist as
 *                         an array of strings or be an empty item in the dictionary.  This may be an empty string to
 *                         set the string array into the item @p baseItem itself.
 *  @param[in] stringArray The array of strings to set in the dictionary.  This may contain a different number of items
 *                         than the existing array.  If the number of items differs, this new array of values will
 *                         replace the existing item at @p path entirely.  If the count is the same as the previous
 *                         item, values will simply be replaced.
 ...
 *
 *  @param[in] id     The @ref IDictionary interface to use to access the items in the dictionary.  This must not be
 *                    `nullptr`.  This must be the same interface that was originally used to create the dictionary @p
 *                    dict.
 *  @param[in] dict   The base item to act as the root of where to set the values relative to.  This may not be
 *                    `nullptr`.
 *  @param[in] argv   The Unix style argument array for the command line to the process.  This must not be `nullptr`.
 *                    The first entry in this array is expected to be the process's name.  Only the arguments starting
 *                    with @p prefix will be parsed here.
 *  @param[in] argc   The Unix style argument count for the total number of items in @p argv to parse.
 *  @param[in] prefix A string indicating the prefix of arguments that should be parsed by this operation.  Arguments
 *                    that do not start with this prefix will simply be\ ignored.
 *  @returns No return value.
 *
 *  @remarks This parses command line arguments to find ones that should be added to a settings dictionary.  Only
 *           arguments beginning with the given prefix will be added.  The type of each individual item added to the
 *           dictionary will be automatically detected based on the same criteria used for
 *           setDictionaryElementAutoType().
 *
 *  @thread_safety This operation itself is thread safe, but a race condition may still exist if multiple threads are
 *                 trying to set the values for the same set of items simultaneously.  The operation will succeed, but
 *                 the value that gets set in each item in the end is undefined.
 */
inline void setDictionaryFromCmdLineS(
    IDictionary* id, Item* dict, char** argv, int argc, carb::cpp::zstring_view prefix = "--/")
{
    carb::extras::CmdLineParser cmdLineParser(prefix.c_str());
...
/**
 * @copydoc setDictionaryFromCmdLineS(IDictionary* id, Item* dict, char** argv, int argc, carb::cpp::zstring_view
 *          prefix)
 */
inline void setDictionaryFromCmdLine(IDictionary* id, Item* dict, char** argv, int argc, const char* prefix)
{
    setDictionaryFromCmdLineS(id, dict, argv, argc, carb::cpp::zstring_view(carb::cpp::unsafe_length, prefix));
}
...
#endif // !CARB_VERSION_ATLEAST(carb_dictionary_IDictionary, 2, 0)
...
{
    setStringArray(dict, item, {}, stringArray);
}
 ...
 */
inline Item* createDictionaryFromFileS(ISerializer* serializer, carb::cpp::zstring_view filename)
{
    carb::filesystem::IFileSystem* fs = carb::getCachedInterface<carb::filesystem::IFileSystem>();

    auto file = filesystem::detail::openFile(fs, filename, filesystem::OpenMode::eRead);
    if (!file)
        return nullptr;

    const size_t fileSize = filesystem::detail::getFileSize(fs, *file).value_or(0);
    ...
    const size_t readBytes = filesystem::detail::readFileChunk(fs, *file, content, contentLen).value_or(0);
    file->reset();

    if (readBytes != fileSize)
    {
        CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Only read %zu bytes of a total of %zu bytes from file '%.*s'",
                       readBytes, fileSize, (int)filename.size(), filename.data());
    }
...
/**
 * @copydoc createDictionaryFromFileS(ISerializer* serializer, carb::cpp::zstring_view filename)
 */
inline Item* createDictionaryFromFile(ISerializer* serializer, const char* filename)
{
    return createDictionaryFromFileS(serializer, carb::cpp::zstring_view(carb::cpp::unsafe_length, filename));
}

/** Writes the contents of a dictionary to a file.
 *
 *  @param[in] serializer        The @ref ISerializer interface to use to format the dictionary data before writing it
 *                               to file.  This may not be `nullptr`.
 *  @param[in] dictionary        The dictionary root item to format and write to file.  This not be `nullptr`.  This
 *                               must have been created by the same @ref IDictionary interface that @p serializer uses
 *                               internally.
 *  @param[in] filename          The name of the file to write the formatted dictionary data to.  This file will be
 *                               unconditionally overwritten.  It is the caller's responsibility to ensure any previous
 *                               file at this location can be safely overwritten.
 *  @param[in] serializerOptions Option flags passed to the @p serializer interface when formatting the dictionary data.
 *  @returns No return value.
 *
 *  @remarks This formats the contents of a dictionary to a string and writes it to a file.  The file will be formatted
 *           according to the serializer interface that is used.  The extra flag options passed to the serializer in @p
 *           serializerOptions control the specifics of the formatting.  The formatted data will be written to the file
 *           as long as it can be opened successfully for writing.
 *
 *  @thread_safety This operation itself is thread safe.  However, if another thread is attempting to modify the
 *                 dictionary @p dictionary at the same time, a race condition may exist and undefined behavior could
 *                 occur.  It won't crash but the written data may be unexpected.
 */
inline void saveFileFromDictionaryS(ISerializer* serializer,
                                    const dictionary::Item* dictionary,
                                    carb::cpp::zstring_view filename,
                                    SerializerOptions serializerOptions)
{
    ...
    auto sFile = filesystem::detail::openFile(fs, filename, filesystem::OpenMode::eWrite);
    if (!sFile)
    {
        CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "failed to open file '%.*s' - unable to save the dictionary",
                       (int)filename.size(), filename.data());
        return;
    }
    (void)filesystem::detail::writeFileChunk(fs, *sFile, serializedString.get(), strlen(serializedString.get()));
}
...
{
    saveFileFromDictionaryS(
        serializer, dictionary, carb::cpp::zstring_view(carb::cpp::unsafe_length, filename), serializerOptions);
}
                                   ...
                                   bool includeRoot = true)
{
    ...
    std::vector<cpp::zstring_view> pathElementsNames;

    ScopedRead lock(*dict, item);
    while (item)
    {
        pathElementsNames.push_back(detail::getItemName(dict, item).value_or(""));
    ...
    for (const auto& elementName : pathElementsNames)
    {
        ...
        totalSize += elementName.size();
    }

    std::string result;
    result.reserve(totalSize);

    for (auto iter = pathElementsNames.rbegin(); iter != pathElementsNames.rend(); ++iter)
    {
        result += '/';
        result += *iter;
    }
...
//! Specialization for a `carb::cpp::string_view` item value.
//! @copydoc toItemType()
template <>
inline ItemType toItemType<cpp::string_view>()
{
    return ItemType::eString;
}

//! Specialization for a `carb::cpp::zstring_view` item value.
//! @copydoc toItemType()
template <>
inline ItemType toItemType<cpp::zstring_view>()
{
    return ItemType::eString;
}
...
inline void unsubscribeTreeFromAllEvents(IDictionary* dict, Item* item)
{
    ...
    walkDictionary(
        dict, WalkerMode::eIncludeRoot, item, 0u, unsubscribeItem, dict,
        [](IDictionary* dict, Item* item, size_t index) { return detail::getItemChildByIndex(dict, item, index); });
}
 ...
 */
inline UpdateAction overwriteOriginalWithArrayHandling(const Item* dstItem,
                                                       [[maybe_unused]] ItemType dstItemType,
                                                       const Item* srcItem,
                                                       [[maybe_unused]] ItemType srcItemType,
...
/** Helper function for IDictionary::update() that merges a tree into a destination favoring originals and ensures that
 *  arrays are properly kept without merging.
 *
 *  @param[in] dstItem              The destination dictionary item for the merge operation.  This
 *                                  may be `nullptr` if the destination item doesn't already exist
 *                                  in the tree (ie: a new item is being merged into the tree).
 ...
 *
 *  @remarks This is intended to be used as the @ref OnUpdateItemFn callback function for the
 *           @ref IDictionary::update() function.  When this is used, the @ref IDictionary interface object must
 *           be passed into the @a userData parameter of IDictionary::update().
 *
 *  @thread_safety This operation is thread safe.  However, this call isn't expected to be used
 *                 directly, but rather through the IDictionary::update() function.  The overall
 *                 thread safety of that operation should be noted instead.
 */
inline UpdateAction mergeKeepOriginalWithArrayHandling(const Item* dstItem,
                                                       [[maybe_unused]] ItemType dstItemType,
                                                       [[maybe_unused]] const Item* srcItem,
                                                       [[maybe_unused]] ItemType srcItemType,
                                                       void* dictionaryInterface)
{
    if (!dstItem)
        return UpdateAction::eOverwrite;
    auto dict = static_cast<IDictionary*>(dictionaryInterface);
    if (dict->getItemFlag(dstItem, ItemFlag::eUnitSubtree))
        return UpdateAction::eKeepSubtree;
