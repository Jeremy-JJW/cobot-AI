...
public:
    ...
    bool sendFd(const int* fds, int fdCount)
    {
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "sendmsg failed, errno=%d/%s", errno, strerror(errno));
            return false;
        }
        return true;
    }
    ...
    bool receiveFd(int* fds, int fdCount)
    {
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "recvmsg failed, errno=%d/%s", errno, strerror(errno));
            return false;
        }
    ...
    bool startClient(const char* socketPath)
    {
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Failed to create socket, errno=%d/%s", errno, strerror(errno));
            return false;
        }
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Failed to connect to socket %s, errno=%d/%s", addr.sun_path,
    ...
    bool startServer(const char* socketPath, int backlog = 10)
    {
        ...
        {
            CARB_LOG_ERROR(
                CARB_LOG_DEFAULT_CHANNEL, "Failed to create server socket, errno=%d/%s", errno, strerror(errno));
            return false;
        }
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "unlinking socket failed, errno=%d/%s", errno, strerror(errno));
            return false;
        }
        ...
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Failed to bind the socket, errno=%d/%s", errno, strerror(errno));
            return false;
        }

        if (listen(m_socket, backlog) == -1)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Failed to listen on socket, errno=%d/%s", errno, strerror(errno));
            return false;
        }

        CARB_LOG_INFO(CARB_LOG_DEFAULT_CHANNEL, "DomainSocket: stream server started at %s", socketPath);
        return true;
    }
    ...
    bool acceptConnection(DomainSocket& server)
    {
        ...
        {
            CARB_LOG_ERROR(
                CARB_LOG_DEFAULT_CHANNEL, "an error on the server socket, errno=%d/%s", errno, strerror(errno));
            return false;
        }
        ...
        if (m_socket == CARB_INVALID_SOCKET)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "accepting connection failed, errno=%d/%s", errno, strerror(errno));
            return false;
        }
        ...
        CARB_LOG_INFO(CARB_LOG_DEFAULT_CHANNEL, "DomainSocket: accepted connection on %s", addr.sun_path);
        return true;
    }
