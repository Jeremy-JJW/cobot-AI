...
// Always include support for casting to omni::string and carb::cpp::string_view
#include "../omni/python/PyString.h"

#include "detail/PybindProlog.h"
#include <pybind11/functional.h>
#include <pybind11/stl.h>
#include "detail/PybindEpilog.h"
...
inline void handlePythonError(const py::error_already_set& e)
{
    ...
    catch (std::exception& ex)
    {
        OMNI_LOG_ERROR(OMNI_LOG_DEFAULT_CHANNEL,
                       "While handling py::error_already_set(%s), another exception occurred: %s", e.what(), ex.what());
    }
    catch (...)
    {
        OMNI_LOG_ERROR(OMNI_LOG_DEFAULT_CHANNEL,
                       "While handling py::error_already_set(%s), a non-std::exception was thrown", e.what());
    }
}
...
auto callPythonCodeSafe(const std::function<Sig>& fn, ArgsT&&... args)
{
    ...
    {
        CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "%s", e.what());
    }
