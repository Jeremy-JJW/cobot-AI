...
/** Flag to indicate that the current thread's traceparent ID should be implicitly passed to the
 *  child process via an environment variable.  When this flag is set, the calling thread's
 *  current traceparent ID will be retrieved and passed to the child process through the
 *  OMNI_OTEL_TRACES_EXTERNAL_TRACE_ID environment variable.  This allows the child process to
 *  continue the trace context from the parent process, enabling distributed tracing across
 *  process boundaries.  If no trace is currently active on the calling thread, the environment
 *  variable will not be set.  This flag has no effect if the child process does not use
 *  OpenTelemetry or does not read the OMNI_OTEL_TRACES_EXTERNAL_TRACE_ID environment variable.
 *  This flag may be combined with explicit environment variable settings in @ref LaunchDesc::env.
 *  If the same environment variable is explicitly set in @ref LaunchDesc::env, the manually
 *  specified value will take precedence over the automatically added traceparent.
 */
constexpr LauncherFlags fLaunchFlagPassTraceparent = 0x00000800;

/** Flag to indicate that the parent process' telemetry session ID should be passed to the child
 *  process on its command line via the `--/telemetry/parentSessionId='\<id\>'` setting.  This
 *  allows the child process to link its telemetry session back to the parent process' session.
 *  If the parent process does not have a valid session ID (ie: structured logging is not
 *  initialized), the argument will not be added.  This flag is automatically implied when the
 *  child process executable image is `kit` or `kit.exe`, or when the executable image is `python`
 *  (or similar) and other `--/` style Carbonite settings are present on the command line.  This
 *  flag is ignored if @ref fLaunchFlagNoPassParentSessionId is also set.
 *
 *  @note When the `/telemetry/parentSessionId` setting is passed to the child process, its value
 *        will be passed as a string instead of a 64-bit integer.  This is done because of a
 *        limitation of the `ISettings` and `IDictionary` interfaces - they are not able to
 *        parse unsigned 64-bit integer values.  Parsed values are clamped to INT64_MAX instead
 *        of allowing the value to wrap to be negative instead.  Passing the value as a string
 *        works around this by keeping the value verbatim instead of trying to parse it as an
 *        integer.  This does however mean that the setting's value needs to be read as a string
 *        and converted to an unsigned integer with something like `std::strtoull()`.  The
 *        convenience helper function `carb::launcher::getParentSessionId()` is also available
 *        to retrieve this value as a uint64_t.
 */
constexpr LauncherFlags fLaunchFlagPassParentSessionId = 0x00001000;

/** Flag to indicate that the parent process' telemetry session ID should not be automatically
 *  passed to the child process, even in cases where it would normally be implied (such as when
 *  launching `kit` or `kit.exe`).  Note that this will override the functionality of the
 *  @ref fLaunchFlagPassParentSessionId flag if also present.
 */
constexpr LauncherFlags fLaunchFlagNoPassParentSessionId = 0x00002000;
/** @} */
 ...
 *
 *  @warning On Windows, specifying the command line interprerter as simply `cmd.exe` as is listed
 *           here can lead to a potential security risk (MS14-019).  This is because a malicious
 *           actor could potentially intercept the `cmd.exe` executable image by placing their own
 *           in the current working directory for the process and allow for arbitrary code
 *           execution.  To avoid this potential security risk, simply do not set a value in the
 *           @ref LaunchDesc::interpreter field and allow `ILauncher` to find the correct
 *           interpreter instead.  The @ref LaunchDesc::interpreter field should only be used
 *           when running a script file that is not a shell or Python script.  If this `cmd.exe`
 *           name must be used, it should instead be given as an absolute path to the interpreter
 *           which has a default install location of `c:\windows\system32\cmd.exe`.  More info
 *           on this potential security issue can be found at
 *           https://msrc.microsoft.com/blog/2014/04/ms14-019-fixing-a-binary-hijacking-via-cmd-or-bat-file/.
 */
#if CARB_PLATFORM_WINDOWS
constexpr char kInterpreterShellScript[] = "cmd.exe /C";
constexpr char kInterpreterShellScript2[] = "cmd.exe /C";
#else
constexpr char kInterpreterShellScript[] = "/bin/sh";

/** @copydoc kInterpreterShellScript */
constexpr char kInterpreterShellScript2[] = "/bin/bash";
#endif
 ...
 */
constexpr char kInterpreterPythonScript[] = "python";

/** @copydoc kInterpreterPythonScript */
constexpr char kInterpreterPythonCommand[] = "python -c";
...
struct LaunchDesc
{
    ...
    /** An optional command interpreter path to use when launching the new process.  This can be
     ...
     *  interpreter could not be found for the named script, or the named script could not
     ...
     *  used instead, it is the caller's responsibility to set this appropriately.  However,
     *  note that the interpreters given in the `kInterpreter*` names are also the same ones
     *  as are auto-detected internally when this value is `nullptr`.
     *
...
struct ILauncher
{
    CARB_PLUGIN_INTERFACE("carb::launcher::ILauncher", 1, 4)
 ...
 */
inline void restoreParentDeathSignal([[maybe_unused]] KillFlags flags = 0)
{
#if CARB_PLATFORM_LINUX
