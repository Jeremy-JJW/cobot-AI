...
// 1. In premake5.lua, make sure `define_rstringversiontest(X)` is called for your your new version number.
...
static_assert(
    sizeof(Data) <= 4096,
    "NOTE: when this stops working, the Data struct has exceeded the size of a page. "
    "This means that newer versions will try to map too much if an older version created the shared memory segment.");
...
inline bool uncasedEqual(const char* str1, const char* str2, size_t len)
{
    for (size_t i = 0; i != len; ++i)
        if (carb::tolower(static_cast<unsigned char>(str1[i])) != carb::tolower(static_cast<unsigned char>(str2[i])))
            return false;
    return true;
}
...
{
    const size_t minlen = ::carb_min(len1, len2);
    for (size_t i = 0; i != minlen; ++i)
    {
        auto c1 = static_cast<signed char>(carb::tolower(static_cast<unsigned char>(str1[i])));
        auto c2 = static_cast<signed char>(carb::tolower(static_cast<unsigned char>(str2[i])));
        if (c1 != c2)
...
public:
    ...
    const Data* cdata() const noexcept
    {
        return m_data;
    }
    ...
    const Rec* at(uint32_t stringId) const
    {
        ...
        return chunkList < m_data->ChunkListSize && m_data->chunkLists[chunkList] ?
                   m_data->chunkLists[chunkList][chunkListOffset] :
                   nullptr;
    }
    ...
    uint32_t findOrAdd(const cpp::string_view sv, const bool uncased, const RStringOp op)
    {
        if (sv.empty())
            return uint32_t(eRString::Empty);
        return findOrAdd(carb::hashLowercaseBuffer(sv.data(), sv.length()), sv, uncased, op);
    }

    uint32_t findOrAdd(const RStringLiteral lit, const bool uncased)
    {
        if (lit.str.empty())
            return uint32_t(eRString::Empty);
        return findOrAdd(lit.hash, lit.str, uncased, RStringOp::eRegister);
    }

    uint32_t findOrAddWithHint(const uint64_t hash, const cpp::string_view sv, eRString enumVal)
    {
        return findOrAddInternal(hash, sv, false, enumVal);
    }
    ...
    uint32_t convertUncased(uint32_t stringId) const
    {
        ...
        if (rec)
        {
            ...
            rec = hashTableFind(rec->m_uncasedHash, cpp::string_view(rec->m_string, rec->m_stringLen), true);
            CARB_ASSERT(rec);
...
private:
    ...
    void init()
    {
        ...
        CARB_ASSERT(sizeNeeded != 0 && (sizeNeeded % allocGranularity) == 0);
...
#define EMPTY_ENTRY(a, b) { CARB_HASH_STRING_LOWER(""), { "" }, eRString(a) },
#define ENTRY(a, b) { CARB_HASH_STRING_LOWER(#b), { #b }, eRString(a) },
...
#undef EMPTY_ENTRY
        ...
        {
            findOrAddWithHint(entries[i].hash, entries[i].sv, entries[i].enumVal);
        }
        ...
        CARB_ASSERT(validate(m_data)); // should validate fine when first initialized
    }
...
#    if CARB_DEBUG
                fprintf(
                    stderr, "Warning: RString shared memory opened but failed to validate! Attempting to re-create.\n");
#    endif
                m_data = nullptr;
                delete m_view;
                m_view = nullptr;
                m_shm.close(true); // Force unlink since validation failed on this shm object
...
#else
        auto p = ::mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
        return p != MAP_FAILED ? p : nullptr;
#endif
    }

    void sysFree(void* mem, [[maybe_unused]] size_t size)
    {
#if CARB_PLATFORM_WINDOWS
    ...
    Rec* hashTableFind(const uint64_t uncasedHash, const cpp::string_view sv, const bool uncased) const
    {
        ...
        // store in findOrAdd().
        Rec* rec =
            carb::cpp::atomic_ref<Rec*>(m_data->hashTableBuckets[size_t(uncasedHash) & (m_data->NumHashBuckets - 1)])
                .load(std::memory_order_acquire);
        for (; rec; rec = rec->m_next)
        {
            if (rec->m_uncasedHash == uncasedHash && sv.length() == rec->m_stringLen)
            {
                if ((!uncased && casedEqual(sv.data(), rec->m_string, sv.length())) ||
                    (uncased && rec->m_authority && uncasedEqual(sv.data(), rec->m_string, sv.length())))
                {
    ...
    CARB_NOINLINE uint32_t findOrAddInternal(const uint64_t uncasedHash,
                                             const cpp::string_view sv,
                                             const bool uncased,
                                             eRString enumVal = kNoHint)
    {
        // Need the mutex for this scope
        lockMutex();
        CARB_SCOPE_EXIT
        {
            unlockMutex();
        ...
        for (Rec* rec = pBucketHead; rec; rec = rec->m_next)
        {
            if (rec->m_uncasedHash == uncasedHash && sv.length() == rec->m_stringLen)
            {
                // We're now looking for a case-insensitive authority, so do an case-insensitive check.
                if (uncasedEqual(sv.data(), rec->m_string, sv.length()))
                {
                    if (rec->m_authority)
                    {
        ...
        size_t sizeNeeded = sizeof(Rec) + sv.length() + 1;
        ...
        if (size_t(m_data->memEnd - m_data->mem) < sizeNeeded)
        {
            CARB_FATAL_UNLESS(sizeNeeded < (m_data->AllocSize - sizeof(MemoryAlloc)),
                              "Trying to register massive string of size %zu!", sv.length());
            allocChunk();
        }

        // If we don't have an existing authority, we want this new Rec to be the uncased authority.
        *ppRec = new (m_data->mem)
            Rec(pBucketHead, uint32_t(index), uint32_t(sv.length()), !authority, uncasedHash, sv.data());
        m_data->mem += sizeNeeded;

        // OMPE-77021: We cannot leave mem == memEnd, otherwise validate() performed by a module load will fail
        if (m_data->mem == m_data->memEnd)
            allocChunk();
        CARB_ASSERT(m_data->mem < m_data->memEnd); // always have space available for next entry
    ...
    uint32_t findOrAdd(const uint64_t uncasedHash, const cpp::string_view sv, const bool uncased, const RStringOp op)
    {
        if (auto rec = hashTableFind(uncasedHash, sv, uncased))
            return rec->m_stringId;
        return op == RStringOp::eFindExisting ? uint32_t(eRString::Empty) : findOrAddInternal(uncasedHash, sv, uncased);
    }
