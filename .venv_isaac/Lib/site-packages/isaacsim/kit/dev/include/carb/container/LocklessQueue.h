...
public:
     ...
     * Default constructor.
     */
    constexpr LocklessQueueLink() noexcept = default;

    CARB_PREVENT_COPY_AND_MOVE(LocklessQueueLink);

private:
    CARB_VIZ std::atomic<T*> m_next{ nullptr };
...
public:
    ...
    bool push(T* p)
    {
        ...
        next(p).store(nullptr, std::memory_order_release);
        return _push(p, p);
    }
    ...
    bool push(InputItRef begin, InputItRef end)
    {
        ...
        next(std::addressof(*last)).store(nullptr, std::memory_order_release);
    ...
    void forEach(Func&& f)
    {
        ...
        for (T *p = head, *n; p; p = n)
        {
            ...
            n = next(p).load(std::memory_order_acquire);
...
private:
    ...
    bool _push(T* first, T* last)
    {
        ...
        if (CARB_LIKELY(token))
        {
            ...
            next(token).store(first, std::memory_order_release);
            return false;
        }
        else
        {
            // Queue was empty; head points to our first item
            m_head.store(first, std::memory_order_release);
            return true;
        }
    }
    ...
    CARB_NOINLINE T* __WAIT_FOR_ENQUEUE__(std::atomic<T*>& ptr)
    {
        T* val;
        int spins = 0;
        while ((val = ptr.load(std::memory_order_acquire)) == nullptr)
        {
            spins++;
            CARB_UNUSED(spins);
    ...
    T* waitForEnqueue(std::atomic<T*>& ptr)
    {
        ...
        {
            if (CARB_LIKELY((val = ptr.load(std::memory_order_acquire)) != nullptr))
                return val;
            CARB_HARDWARE_PAUSE();
        }
