...
public:
    ...
    void wait(arrival_token&& arrival) const
    {
        ...
        CARB_RELEASE_ASSERT(arrival.m_token != 0, "barrier::wait: Precondition failure: Invalid arrival_token");
        uint64_t data = m_emo.second.load(std::memory_order_acquire);
        uint32_t phase = uint32_t(data >> kPhaseBitShift);
        CARB_RELEASE_ASSERT((phase - arrival.m_token) <= 1,
                            "barrier::wait: Precondition failure: arrival %u is not the previous or current phase %u",
    ...
    void arrive_and_wait()
    {
        ...
        // - We don't assert that the phase is the current or preceding one since it is guaranteed
    ...
    void arrive_and_drop()
    {
        ...
        CARB_RELEASE_ASSERT(
            prev != 0,
            "barrier::arrive_and_drop: Precondition failure: expected count for the current barrier phase must be greater than zero");
...
private:
    ...
    {
        CARB_RELEASE_ASSERT(
            update > 0 && update <= max(), "barrier::arrive: Precondition failure: Invalid update %td", update);

        uint64_t pre = m_emo.second.fetch_sub(uint32_t(update), std::memory_order_acq_rel);
        CARB_RELEASE_ASSERT(ptrdiff_t(int32_t(uint32_t(pre & kCounterMask))) >= update,
                            "barrier::arrive: Precondition failure: update <= count");
