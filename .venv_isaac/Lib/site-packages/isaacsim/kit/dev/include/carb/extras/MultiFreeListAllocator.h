...
public:
    ...
    void* allocate(size_t size)
    {
        ...
        {
            originalChunk = static_cast<uint8_t*>(std::malloc(originalChunkSize));
        }

        if (originalChunk == nullptr)
        {
            CARB_LOG_ERROR(CARB_LOG_DEFAULT_CHANNEL, "Failed to allocate memory!");
            return nullptr;
        }
    ...
    void* allocateAligned(size_t size, size_t alignment)
    {
        ...
        {
            originalChunk = static_cast<uint8_t*>(std::malloc(originalChunkAlignedSize));
        }
        ...
        CARB_RELEASE_ASSERT(ptrOffset + size <= originalChunkAlignedSize); // CARB_ASSERT?
