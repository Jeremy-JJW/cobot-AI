...
public:
     ...
     */
    bool operator==(const ArgCollector& rhs) const
    {
     ...
     *  @returns A reference to this object suitable for chaining other operators or calls.
     */
    ArgCollector& add(carb::cpp::string_view value)
    {
    ...
    ArgCollector& add(cpp::unbounded_string value)
    {
        m_args.push_back(value.data());
        return *this;
    }
#endif

    //! @copydoc add(carb::cpp::string_view)
...
#ifndef DOXYGEN_BUILD
    ArgCollector& operator+=(cpp::unbounded_string value)
    {
        return add(value);
    }
#endif
    ...
    //! @copydoc add(carb::cpp::string_view)
    ArgCollector& operator+=(cpp::string_view value)
    {
        return add(value);
    }
...
#undef ADD_PRIMITIVE_HANDLER
     ...
     */
    ArgCollector& add(cpp::string_view root,
                      ...
                      void* context = nullptr)
    {
        ...
        auto addSetting = [&](cpp::zstring_view path, int32_t elementData) -> int32_t {
            dictionary::ItemType type;

            type = settings::detail::getItemType(settings, path);
            ...
            if (predicate != nullptr && !predicate(path.c_str(), context))
                return elementData + 1;

            // Optional use is safe here because this call comes from walkSettings()
            switch (type)
            {
                case dictionary::ItemType::eBool:
                    format("%.*s%s=%s", (int)prefix.size(), prefix.data(), &path[1],
                           *settings::detail::get<bool>(settings, path) ? "true" : "false");
                    break;

                case dictionary::ItemType::eInt:
                    format("%.*s%s=%" PRId64, (int)prefix.size(), prefix.data(), &path[1],
                           *settings::detail::get<int64_t>(settings, path));
                    break;

                case dictionary::ItemType::eFloat:
                    format("%.*s%s=%g", (int)prefix.size(), prefix.data(), &path[1],
                           *settings::detail::get<double>(settings, path));
                    break;

                case dictionary::ItemType::eString:
                    format("%.*s%s=\"%s\"", (int)prefix.size(), prefix.data(), &path[1],
                           settings::detail::getStringView(settings, path)->c_str());
                    break;
        ...
        if (rootPath.size() > 1 && rootPath.back() == '/')
            rootPath.remove_suffix(1);

        // walk the settings tree to collect all the requested settings.
        settings::detail::walkSettings(
            dictionary, settings, dictionary::WalkerMode::eIncludeRoot, rootPath, 0, addSetting);
        return *this;
    }
...
public:
    ...
    /** The platform specific separator character to be used in environment variables such as
     *  `PATH` or `PYTHONPATH`.
     */
    static inline constexpr char kPathVarSeparator[] = ";";
#else
    static inline constexpr char kPathVarSeparator[] = ":";
#endif
     ...
     *  @returns `false` if the argument lists in the two objects differ.
     */
    bool operator==(const EnvCollector& rhs) const
    {
...
#undef ADD_PRIMITIVE_HANDLER
    ...
    /** Tests whether a given environment variable exists in this object.
     *
     *  @param[in] name     The name of the variable to check for in this environment block
     *                      object.  This may not be `nullptr` or an empty string.
     *  @returns `true` if an environment variable with the given name exists in this environment
     *           block object.  This will still succeed if the variable exists with no value.
     *           Returns `false` if the named environment variable does not exist in this
     *           environment block.
     */
    ...
    /** Returns a string of all environment variables for debugging purposes.
     *  @returns A `std::string` of all environment variables concatenated
    ...
    std::string toString() const
    {
        ...
        for (auto& var : m_env)
        {
            stream << "    " << var.first << "=" << var.second << std::endl;
        }
...
struct ProcessHandleDeleter
{
    ...
    {
        auto launcher = carb::getCachedInterface<ILauncher>();

        if (launcher != nullptr)
        {
            launcher->destroyProcessHandle(proc);
        }
    }
};
...
/** A scoped handle to a process handle object that automatically destroys the process handle
 *  when it goes out of scope.  This class inherits from std::unique_ptr with a custom deleter
 *  to handle process cleanup.
 *
 *  @note Once a process handle is assigned to this object, it is fully owned and
 *        managed by this object.  It may still be accessed directly using the
 *        get() method or cast operator, but must not be destroyed directly otherwise
 *        the behavior is undefined.  The contained handle can be released from this
 *        object using the release() method, but it is then the caller's responsibility
 *        to destroy the handle when it is no longer needed.
 */
...
public:
     ...
     *
     *  @param[in] proc The process handle to take ownership of.  May be `nullptr`.
     */
    ScopedProcessHandle(Process* proc) noexcept : Base(proc)
    {
    }

    /** Implicit conversion operator to get the raw process handle.
     *
     *  @returns The underlying process handle pointer, which may be `nullptr`.
     */
    operator Process*() const noexcept
    {
        return get();
    }
     ...
     *
     *  @param[in] proc The process handle to take ownership of.  May be `nullptr`.  If this
     *                  object already owns a handle, it will be destroyed before the new
     *                  handle is assigned.
     *  @returns Reference to this object.
     */
    ScopedProcessHandle& operator=(Process* proc) noexcept
    {
        reset(proc);
        return *this;
    }
 ...
 *
 *  @returns The session ID of this process' parent process if available.  This value is
 *           retrieved from the `/telemetry/parentSessionId` setting.  This setting is
 *           expected to be present as a string value when provided.  This must be
 *           provided by the parent process when this process is launched.  If the
 *           parent process' session ID cannot be parsed or is not available, 0 will
 *           be returned instead.
 */
inline omni::structuredlog::SessionId getParentSessionId()
{
    ...
    auto value = carb::settings::detail::getStringView(settings, "/telemetry/parentSessionId");
    ...
    return std::strtoull(value->c_str(), &endptr, 10);
}
