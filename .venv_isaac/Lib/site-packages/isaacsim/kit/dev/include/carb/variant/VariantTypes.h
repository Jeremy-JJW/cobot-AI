...
{
struct Item;
//! A special conversion type for carb::variant::Variant::convertTo instructing it to make a deep copy of the dictionary
//! Item.
struct ManagedItem
{
};
...
constexpr RString eDictionaryCopy{ carb::eRString::RS_dictionarycopy }; //!< A special conversion type.
...
constexpr RString eCharPtrCopy{ carb::eRString::RS_charptrcopy }; //!< Represents `char*` or `const char*`
//! @}
...
struct VTable
{
     ...
     * @param self The VariantData performing the conversion. Can assume that `self->vtable->ConvertTo` is the same as
     * `ConvertTo`.
    ...
    /**
     * Called to attempt to convert to \p desired *from* \p from. A \c nullptr function is the same as returning false.
     * @details If `ConvertFrom` recognizes the v-table `from->vtable` and can convert to that type,
     * then the function should write to \p out and return \c true. \p desired should never be the same as
     * `from->vtable`.
     * @warning If \c false is returned, \p out is in an undefined state. If and only if \c true is returned,
     * \ref traits::destruct() must be called at some later point on \c *out.
     * @note Generally speaking, \ref Equals() and \ref ConvertFrom() should understand the same types.
     * @note In the past, only \ref ConvertTo was supported. This function is typically only called if \ref ConvertTo
     * fails. If two types are mutually convertible via their ConvertTo methods, they need not implement this function.
     * @param desired A v-table representing the type to attempt to convert to. `ConvertFrom` is the same as
     * `desired->ConvertFrom`. Given this, \p desired can often be ignored.
     * @param out If \c true is returned from the function, this must be a valid \ref VariantData and must be later
     * destroyed with \ref traits::destruct(). If \c false is returned then the state of \p out is undefined. There is
     * no requirement that `out->vtable` matches \p desired if \c true is returned; it must merely be valid.
     * @param from The VariantData to convert *from*.
     * @returns \c true if and only if \p out contains a valid converted representation of \p from; \c false otherwise.
     */
...
public:
     ...
     *
     * @note For empty arrays, this may return nullptr depending on the implementation.
     * Always check size() before dereferencing the returned pointer.
     *
     * @returns the beginning of the underlying array, or nullptr for empty arrays (implementation-defined).
     */
 ...
 *     * NOTE: `enum class` types lose type safety since they are stored internally as their underlying type and no type
 *       checking is performed when reading the value out of the Variant.
 ...
 *     * Equality compares via `operator ==` for \ref omni::string, and comparable with `[const] char*`.
 *   - \c std::string
 *     * Internally stored as \ref omni::string and converted to `std::string` when read, since `std::string` is not
 ...
 *   - \ref dictionary::Item pointers
 *     * Stores the pointer, so memory lifetime must be longer than the Variant.
 *     * Copying the variant will trivially copy the pointer. **NOTE**: If a deep copy of the `Item` is desired, you can
 *       use the special \ref carb::dictionary::ManagedItem type with \ref Variant::convertTo() which will produce a
 *       \ref Variant which holds a deep copy of the \ref dictionary::Item. Example:
 *       `auto v2 = v.convertTo<carb::dictionary::ManagedItem>(); auto item = v2.getValue<dictionary::Item*>();`
 *     * Comparison will trivially compare the pointer.
 *     * Can be converted to \c bool as if by \ref dictionary::IDictionary::getAsBool().
 *     * Can be converted to integer types as if by \ref dictionary::IDictionary::getAsInt64(). If the result would not
 *       fit into the integer type, conversion fails.
 *     * Can be converted to \c float or \c double as if by \ref dictionary::IDictionary::getAsFloat() or
 *       \ref dictionary::IDictionary::getAsFloat64().
 *     * Can be converted to `const char*` if and only if the Item represents a string. NOTE however that this merely
 *       trivially copies the string pointer, so it is not recommended unless the lifetime of the Item is guaranteed.
 *     * Can be converted to \ref omni::string or `std::string` as if by \ref dictionary::IDictionary::getStringView()
 *       or if that fails, \ref dictionary::IDictionary::createStringFromItemName().
 *     * Can be converted to \ref VariantArray if \ref dictionary::IDictionary::isAccessibleAsArray() returns true.
 *       This also enables conversion to standard library containers such as `std::vector`. The entries will be
 *       \ref Variant of type `dictionary::Item*`
 *     * Items of type \ref dictionary::ItemType::eDictionary can be converted to \ref VariantMap. The key will be of
 *       type \ref RStringKey. Array indexes will be Empty but will have a number component corresponding to the array
 *       index. The mapped value will be \ref Variant of type `dictionary::Item*`.
 ...
 *     * Usable as long as the contained type has a `Translator` specialization.
 *     * Stored internally as a \ref VariantArrayPtr (see above).
 *   - `std::map` / `std::unordered_map`
 *     * Usable as long as the contained key and value types have `Translator` specializations.
 *     * Stored internally as a \ref VariantMapPtr (see above).
 *   - \ref RString / \ref RStringU / \ref RStringKey / \ref RStringUKey
 *     * RString Types are all comparable with each other. Non-Key types will promote to Key. If either type is Uncased,
 *       the other type is converted to Uncased before comparing.
 ...
 *     * RStringKey can convert to RStringUKey as via `toUncased()`. If the number component is zero it can also convert
 *       to RString and RStringU, otherwise the conversion to these types fails.
 ...
 *     * RStringKey and RStringUKey with number components of zero can convert to `const char*` as if by
 *       `.truncate().c_str()`. If the number component is non-zero, conversion fails.
 ...
 *       provide type safety.
 *   - Python types: If the *carb.variant.python* bindings are loaded, a type is registered that can store Python
 *     objects in \ref Variant.
 *     * Will do a rich comparison with similar types. So a Python `str` will compare with \ref Variant string types
 *       such as `omni::string`, `[const] char*` and `std::string`. Python numeric types will compare with \ref Variant
 *       objects containing numeric types. Same with boolean types.
 *     * \ref Variant::toString will attempt to convert the Python object to a string as by `str()`.
 *     * Python types attempt to convert to their C++ types: `str` to \ref Variant string types, Python numeric types to
 *       C++ numeric types, etc. A Python `sequence` object is convertible to \ref VariantArray. A Python `dict` object
 *       is convertible to \ref VariantMap.
 *     * C++ types are normally not convertible to Python types, however, *carb.eventdispatcher.python* bindings will
 *       perform some translation for events, essentially the reverse of the above list.
 *
