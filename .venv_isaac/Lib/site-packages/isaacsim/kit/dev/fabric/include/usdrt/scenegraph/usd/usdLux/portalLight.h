...
public:
    ...
    static UsdLuxPortalLight Define(const UsdStageRefPtr& stage, const SdfPath& path);
...
public:
    ...
    /// | ||
    /// | -- | -- |
    /// | Declaration | `uniform token light:shaderId = "PortalLight"` |
    /// | C++ Type | TfToken |
...
inline const TfToken UsdLuxPortalLight::_GetStaticTfType()
{
    const static TfToken token("UsdLuxPortalLight");
    return token;
}

/* virtual */
inline const TfToken UsdLuxPortalLight::_GetTfTypeToken() const
{
    const static TfToken token("UsdLuxPortalLight");
    return token;
}
...
/* static */
inline UsdLuxPortalLight UsdLuxPortalLight::Define(const UsdStageRefPtr& stage, const SdfPath& path)
{
    if (!stage)
    {
        // TODO error
    ...
    return UsdLuxPortalLight(stage->DefinePrim(path, _GetStaticTfType()));
}


inline UsdAttribute UsdLuxPortalLight::GetLightShaderIdAttr() const
{

    return GetPrim().GetAttribute(TfToken(UsdLuxTokens->lightShaderId));
}

inline UsdAttribute UsdLuxPortalLight::CreateLightShaderIdAttr() const
{
