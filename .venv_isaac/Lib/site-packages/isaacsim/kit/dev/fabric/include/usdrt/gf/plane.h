...
//
// Note: This source derived from plane.h and plane.cpp in USD and modified for this application.
#pragma once
...
public:
    ...
    GfPlane(const usdrt::GfVec3d& normal, double distanceToOrigin)
    {
    ...
    /// unit length first.
    GfPlane(const usdrt::GfVec3d& normal, const usdrt::GfVec3d& point)
    {
        Set(normal, point);
    }
    ...
    /// collinear.
    GfPlane(const usdrt::GfVec3d& p0, const usdrt::GfVec3d& p1, const usdrt::GfVec3d& p2)
    {
        Set(p0, p1, p2);
    }
    ...
    GfPlane(const usdrt::GfVec4d& eqn)
    {
        Set(eqn);
    }
    ...
    /// length first.
    void Set(const usdrt::GfVec3d& normal, double distanceToOrigin)
    {
    ...
    void Set(const usdrt::GfVec3d& normal, const usdrt::GfVec3d& point);
    ...
    void Set(const usdrt::GfVec3d& p0, const usdrt::GfVec3d& p1, const usdrt::GfVec3d& p2);

    /// This method sets this to the plane given by the equation
    /// \p eqn[0] * x + \p eqn[1] * y + \p eqn[2] * z + \p eqn[3] = 0.

    void Set(const usdrt::GfVec4d& eqn);

    /// Returns the unit-length normal vector of the plane.
    const usdrt::GfVec3d& GetNormal() const
    {
        return _normal;
    }
    ...
    usdrt::GfVec4d GetEquation() const;
    ...
    /// normal.
    double GetDistance(const usdrt::GfVec3d& p) const
    {
        return p * _normal - _distance;
    }

    /// Return the projection of \p p onto the plane.
    usdrt::GfVec3d Project(const usdrt::GfVec3d& p) const
    {
    ...
    GfPlane& Transform(const usdrt::GfMatrix4d& matrix);

    /// Flip the plane normal (if necessary) so that \p p is in the positive
    /// halfspace.
    void Reorient(const usdrt::GfVec3d& p)
    {
        if (GetDistance(p) < 0)
        {
            _normal = -_normal;
    ...
    bool IntersectsPositiveHalfSpace(const usdrt::GfRange3d& box) const;

    /// Returns true if the given point is on the plane or within its positive
    /// half space.
    bool IntersectsPositiveHalfSpace(const usdrt::GfVec3d& pt) const
    {
...
inline bool GfFitPlaneToPoints(const std::vector<usdrt::GfVec3d>& points, GfPlane* fitPlane);

inline void GfPlane::Set(const usdrt::GfVec3d& normal, const usdrt::GfVec3d& point)
{
    _normal = normal.GetNormalized();
    _distance = GfDot(_normal, point);
}

inline void GfPlane::Set(const usdrt::GfVec3d& p0, const usdrt::GfVec3d& p1, const usdrt::GfVec3d& p2)
{
    _normal = GfCross(p1 - p0, p2 - p0).GetNormalized();
    _distance = GfDot(_normal, p0);
}

inline void GfPlane::Set(const usdrt::GfVec4d& eqn)
{
...
inline usdrt::GfVec4d GfPlane::GetEquation() const
{
    return usdrt::GfVec4d(_normal[0], _normal[1], _normal[2], -_distance);
}

inline GfPlane& GfPlane::Transform(const usdrt::GfMatrix4d& matrix)
{
    ...
    // incorrectly.
    const usdrt::GfMatrix4d adjoint = matrix.GetInverse().GetTranspose();
    Set(GetEquation() * adjoint);

    return *this;
}

inline bool GfPlane::IntersectsPositiveHalfSpace(const usdrt::GfRange3d& box) const
{
    if (box.IsEmpty())
        return false;
...
inline bool GfFitPlaneToPoints(const std::vector<usdrt::GfVec3d>& points, GfPlane* fitPlane)
{
    ...
    if (points.size() < 3)
    {
        // error Need three points to correctly fit a plane
        return false;
    }

    // We'll use the centroid of the points as the origin of our fit plane.
    usdrt::GfVec3d sumOfPoints(0.0);
    for (const usdrt::GfVec3d& p : points)
    {
        sumOfPoints += p;
    }
    const usdrt::GfVec3d centroid = sumOfPoints / points.size();
    ...
    for (const usdrt::GfVec3d& p : points)
    {
        const usdrt::GfVec3d offset = p - centroid;
    ...
    usdrt::GfVec3d equation;
    if (det1 > 0.0 && det1 > det2 && det1 > det3)
    {
        // A^T B = {{\sum (y_i) (-x_i)},
        //          {\sum (z_i) (-x_i)}}
        // X = {{b}, {c}}
        const usdrt::GfVec2d atb1(-xy, -xz);
        const usdrt::GfVec2d leastSquaresEstimate = ata1.GetInverse() * atb1;
        equation[0] = 1.0;
    ...
    else if (det2 > 0.0 && det2 > det3)
    {
        ...
        //          {\sum (z_i) (-y_i)}}
        // X = {{a}, {c}}
        const usdrt::GfVec2d atb2(-xy, -yz);
        const usdrt::GfVec2d leastSquaresEstimate = ata2.GetInverse() * atb2;
    ...
    else if (det3 > 0.0)
    {
        ...
        //          {\sum (y_i) (z_i)}}
        // X = {{a}, {b}}
        const usdrt::GfVec2d atb3(-xz, -yz);
        const usdrt::GfVec2d leastSquaresEstimate = ata3.GetInverse() * atb3;
        equation[0] = leastSquaresEstimate[0];
    ...
    fitPlane->Set(usdrt::GfVec4d(equation[0], equation[1], equation[2], d));
    return true;
}
