...
public:
    ...
    static UsdGeomXform Define(const UsdStageRefPtr& stage, const SdfPath& path);
...
inline const TfToken UsdGeomXform::_GetStaticTfType()
{
    const static TfToken token("UsdGeomXform");
    return token;
}

/* virtual */
inline const TfToken UsdGeomXform::_GetTfTypeToken() const
{
    const static TfToken token("UsdGeomXform");
    return token;
}
...
/* static */
inline UsdGeomXform UsdGeomXform::Define(const UsdStageRefPtr& stage, const SdfPath& path)
{
    if (!stage)
    {
        // TODO error
    ...
    return UsdGeomXform(stage->DefinePrim(path, _GetStaticTfType()));
}
