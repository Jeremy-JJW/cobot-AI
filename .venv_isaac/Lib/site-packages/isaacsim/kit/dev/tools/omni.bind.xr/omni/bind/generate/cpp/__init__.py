...
from omni.bind import extract, model  # type: ignore[attr-defined]
...
"""
    ...
    def _convert_interface(self, interface):
        ...
        for method in interface.methods:
            ...
            post_abi_call = ""
            early_post_abi_call = ""
            optional_noexcept = "noexcept"
            optional_const = "const" if method.is_const else ""
            throws = False

            return_prefix, return_type, return_suffix = self._convert_return(method)

            # check if the return type wants to be translated to an exception
            ret_attributes = extract.get_attributes(method.cursor.type.get_result().get_declaration(), None)
            if ret_attributes:
                translate_to_exception = ret_attributes.translate_to_exception
            else:
                translate_to_exception = None

            # check if the method wants the return translated to an exception
            # can also be used to undo the default for a type
            if method.attributes.translate_to_exception:
                translate_to_exception = method.attributes.translate_to_exception

            if translate_to_exception:
                return_type = "void"
                return_types = []
                return_var_names = []
                early_post_abi_call += f"{translate_to_exception}(return_);\n"
                optional_noexcept = ""
            else:
            ...
            for i in range(len(method.parameters)):
                ...
                if len(parameters_text) > 0:
                    parameters_arr.append(parameters_text)
                    decl_parameters_arr.append(parameters_text + default_arg_text)
                pre_abi_call += pre_call_text
                if len(call_text) > 0:
                    abi_call_args_arr.append(call_text)
                post_abi_call += post_call_text

                throws = throws or method_throws
                if return_type_text:
                    return_types.append(return_type_text)
                    return_var_names.append(return_var_name)
                elif return_var_name:
                    self._reporter.error(
                        param.cursor, f"Had some return var name without a type override: {return_var_name}"
                    )

            decl_parameters = ", ".join(decl_parameters_arr)
            parameters = ", ".join(parameters_arr)
            abi_call_parameters = ", ".join(abi_call_args_arr)
            ...
            post_abi_call = early_post_abi_call + post_abi_call
            return_type_names = [x if isinstance(x, str) else extract.get_full_type_name(x) for x in return_types]

            if (len(pre_abi_call) == 0) and (len(post_abi_call) == 0):
                if len(return_type_names) > 1:
                    self._reporter.error(
                        param.cursor, "omni.bind coding error: should have pre/post-abi calls for c++ multi-return"
                    )

                if method.returns_void or method.throws_result:
                    abi_call = f"{abi_call};"
                elif method.can_return_reference:
                    abi_call = f"return *({abi_call});"
                else:
                    abi_call = f"return {return_prefix}{abi_call}{return_suffix};"
            else:
                return_var = "return_"
                if len(return_type_names) > 1:
                    return_type = f"std::tuple<{', '.join(return_type_names)}>"
                    return_var = f"{return_type} {{ {', '.join(return_var_names)} }}"
                elif len(return_type_names) == 1:
                    return_type = return_type_names[0]
                    return_var = return_var_names[0]
                ...
                if method.returns_void or method.throws_result:
                    ...
                    abi_call = f"auto return_ = {return_prefix}{abi_call}{return_suffix};"

                if len(return_type_names) == 0:
                    if post_abi_call:
                        post_abi_call = "\n" + post_abi_call
                    return_type = "void"
                else:
                    post_abi_call = f"\n{post_abi_call}return {return_var};"
            ...
            decl += f"""
                {method.comment}
                {return_type} {method.name}({decl_parameters}) {optional_const} {optional_noexcept};
            """

            impl += f"""
                inline {return_type} omni::core::Generated<{interface.abi_name}>::{method.name}({parameters}) {optional_const} {optional_noexcept}
                {{
    ...
    def _convert_return(self, method):
        if method.throws_result:
            ...
            return self._convert_type_as_return(method.cursor, method.cursor.type.get_result(), method.attributes)

    def _convert_type_as_return(self, cursor, return_type, attributes):
        if attributes.output_convert_prefix or attributes.output_convert_type or attributes.output_convert_suffix:
            return (
                attributes.output_convert_prefix or "",
                attributes.output_convert_type or extract.get_full_type_name(return_type),
                attributes.output_convert_suffix or "",
            )

        if attributes.bool:
            return "", "bool", " != 0"
        if attributes.not_null:
            if not extract.is_pointer(return_type):
                self._reporter.error(cursor, "'not_null' return must have a pointer type to convert into a reference")
            if return_type.get_pointee().is_const_qualified():
                return "*", f"const {extract.get_full_type_name(return_type.get_pointee())}&", ""
            else:
                return "*", f"{extract.get_full_type_name(return_type.get_pointee())}&", ""
        if extract.is_pointer(return_type):
            # if method.returns_interface_pointer:
            if self._hierarchy.is_interface(return_type.get_pointee().get_canonical().get_declaration()):
                return (
                    "omni::core::steal(",
                    f"omni::core::ObjectPtr<{extract.get_full_type_name(return_type.get_pointee())}>",
                    ")",
                )
            # elif method.returns_kit_interface_pointer:
            elif self._hierarchy.is_kit_interface(return_type.get_pointee().get_canonical().get_declaration()):
                return (
                    "carb::stealObject(",
                    f"carb::ObjectPtr<{extract.get_full_type_name(return_type.get_pointee())}>",
                    ")",
                )

        return "", extract.get_full_type_name(return_type), ""
    ...
    def _convert_parameter(self, method, param):
        ...
        if param.attributes.default:
            default_arg_text = f" = {param.attributes.default}"
        ...
        if param.is_returned:
            ...
            if param.attributes.default:
                self._reporter.error(param.cursor, "Cannot have default value for a variadic list")
            ...
            and not param.attributes.input
            and not param.attributes.count_of
            and (extract.is_pointer(param.cursor.type) or extract.is_reference(param.cursor.type))
        ):
            if default_arg_text:
                self._reporter.error(param.cursor, "Cannot have default value for out param")

            parameters_text = ""
            return_type_text = param._pointee_type
            return_var_name = f"out_{param.name}"
            pre_call_text = f"{param.pointee_name} {return_var_name};"
            if extract.is_pointer(param.cursor.type):
                call_text = f"&{return_var_name}"
            elif extract.is_reference(param.cursor.type):
                call_text = f"{return_var_name}"
            else:
                self._reporter.error(param.cursor, "Out param must be ref or pointer type")

            ret_prefix, return_type_text, ret_suffix = self._convert_type_as_return(
                param.cursor, return_type_text, param.attributes
            )

            if ret_prefix or ret_suffix:
                post_call_text = (
                    f"{return_type_text} converted_{param.name} = {ret_prefix}{return_var_name}{ret_suffix};"
                )
                return_var_name = f"converted_{param.name}"
        else:
...
class PyGenerator(CodeGenerator):
    ...
    def __init__(
        ...
        *,
        ...
        self._opaque_structs: list = []
        self._written_opaque_structs: dict = {}
...
"""
    ...
    def _get_comment(self, obj):
        ...
        # Process line by line for clarity and safety
        lines = comment.splitlines(keepends=True)
        processed_lines = []
        for line in lines:
            # Remove leading whitespace, then leading /* or /** or */ or *
            line = re.sub(r"^[ \t]*(?:/\*\*?|\*/|\*(?!/))", "", line)
            # Remove trailing */ and any whitespace after it (preserving newline if present)
            line = re.sub(r"\*/[ \t]*(\n?)$", r"\1", line)
            processed_lines.append(line)
        comment = "".join(processed_lines)
