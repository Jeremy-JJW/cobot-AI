...
class Attributes(object):
    ...
    def __init__(self, cursor):
        ...
        attrs = {
            ...
            # API may convert to ref instead of pointer [TODO - convert to `ref`]
            "not_null" : False,
            ...
            # if set, treat an enum as a type that should be translated into an exception, using a given function.
            # this value should be a function that takes the enum type as a parameter and throws an exception if needed
            "translate_to_exception": None,

            # treat an int-like param/return as a bool
            "bool": False,

            # add a default value to the API
            "default": None,

            # some conversion code, i.e. "std::string(" from `std::string(my_str)`
            "output_convert_prefix": None,

            # actual return type, i.e. "std::string" for `std::string(my_str)`
            "output_convert_type": None,

            # some conversion code, i.e. ")" from `std::string(my_str)`
            "output_convert_suffix": None,
    ...
    def _parse(self, cursor):
        ...
        # Deal with templates with multiple arguments
        split_arr = s.split(",")
        filtered_arr = []
        x = 0
        while x < len(split_arr):
            val = split_arr[x]
            if "<" in val:
                val = ""
                while x < len(split_arr):
        ...
        tokens = [x.strip() for x in filtered_arr]
        for t in tokens:
            if t.find("=") != -1:
...
class Parameter(CursorObject):
    ...
    def _validate_attributes(self, reporter: DiagnosticReporter):
        ...
        indirection = ""
        while not (t is None):
            if not self.hierarchy.is_interface_pointer(t) and not self.hierarchy.is_kit_interface_pointer(t):
                if not extract.is_abi_safe_type(self.hierarchy, attributes, t, reporter):
                    reporter.error(cursor, f"type is not ABI safe: {t.spelling}")
    ...
    def _pointee_type(self):
        ...
        assert extract.is_pointer(self.cursor.type) or extract.is_reference(self.cursor.type)
    ...
    def is_pointer_to_interface_pointer(self):
        """Returns True if the parameter is a pointer to an interface pointer (useful for output pointers)."""
...
class Method(CursorObject):
    ...
    def can_return_reference(self):
        """Returns true if the return type is a pointer and has the "ref" attribute set."""
...
class Interface(CursorObject):
    ...
    def __init__(self, cursor, hierarchy, reporter: DiagnosticReporter):
        ...
        for child in cursor.get_children():
            if (
                clang.cindex.CursorKind.CXX_METHOD == child.kind
                and clang.cindex.StorageClass.STATIC != child.storage_class
            ):
...
class Field(CursorObject):
    ...
    def __init__(self, cursor, hierarchy, reporter: DiagnosticReporter):
        ...
        if not extract.is_abi_safe_type(self.hierarchy, self.attributes, cursor.type, reporter):
            diagnostic_message = reporter.fatal(cursor, "is not ABI safe type")
            if diagnostic_message:
                raise ValueError(diagnostic_message.message)
