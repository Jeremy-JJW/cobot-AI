...
local function resolve_flags(prj, flags)
    ...
    for _, v in ipairs(prj.externalincludedirs) do
        out = out.." --isystem "..relpath(v, prj)
    end
...
local function on_project(prj)
    local script_dir = path.getdirectory(debug.getinfo(1, "S").source:sub(2))
    prj.omnibindpath = script_dir
...
local function patch_on_project()
    local action = premake.action.current()
    local prev_on_project = action.onProject;
    if prev_on_project == nil then
        premake.error("[Error] onProject for current action [" .. action.description .. "] is nil")
    else
        action.onProject = function(prj)
            prev_on_project(prj)
            if string.find(prj.name, "omni.kit.xr.core") ~= nil then
                -- points to custom omni.bind for XR projects
...
function omnibind_xr(tbl)
    kind "StaticLib"
    omnibindfiles(resolvefiles(tbl))
    files { "%{root}/tools/omni.bind.xr/cpp/Dummy.cpp" }
    if os.target() == "windows" then
...
if premake['patchomnibind'] == nil then
    patch_on_project()
    premake['patchomnibind'] = true
end