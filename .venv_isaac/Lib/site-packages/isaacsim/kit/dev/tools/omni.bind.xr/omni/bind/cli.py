...
def main():
    ...
    parser.add_argument("--std", type=str, default="c++17", help="C++ language standard.")
    parser.add_argument(
    ...
    try:
        ...
        for d in translation_unit.diagnostics:
            diag_msg = DiagnosticMessage.from_clang(d)
            reporter.emit_message(diag_msg)
            if diag_msg.level == DiagnosticLevel.FATAL:
                abort_generation = True

        if abort_generation:
            reporter.fatal(None, f"Fatal error in {in_filename}, generation aborted.")
            return 1
