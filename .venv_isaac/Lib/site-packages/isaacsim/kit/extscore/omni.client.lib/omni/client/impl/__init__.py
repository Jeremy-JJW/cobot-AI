...
# See note in generate_init.py for context.
try:
...
def send_message(join_request_id: int, content: bytes) -> Result:
    ...
    Deprecated: channels are deprecated and must not be used.
    Use :py:func:`omni.client.live_send_message` with :py:func:`omni.client.live_join_session_with_callback` instead."""
...
async def send_message_async(join_request_id: int, content: bytes) -> Result:
    ...
    Deprecated: channels are deprecated and must not be used.
    Use :py:func:`omni.client.live_send_message_async` with :py:func:`omni.client.live_join_session_with_callback` instead."""
...
def live_list_sessions(base_layer_url: str) -> Tuple[Result, Tuple[LiveSessionInfo]]:
    """Blocking version of :py:func:`omni.client.live_list_sessions_with_callback`"""

    ret = None

    def live_list_sessions_cb(result, sessions):
        nonlocal ret
        ret = (result, sessions)

    request = live_list_sessions_with_callback(base_layer_url=base_layer_url, callback=live_list_sessions_cb)
    request.wait()
    return ret


async def live_list_sessions_async(base_layer_url: str) -> Tuple[Result, Tuple[LiveSessionInfo]]:
    """Asynchronous version of :py:func:`omni.client.live_list_sessions_with_callback`"""

    loop = asyncio.get_event_loop()
    f = loop.create_future()

    def live_list_sessions_cb(result, sessions):
        loop.call_soon_threadsafe(set_result_if_not_done, f, (result, sessions))
...
def live_create_session(base_layer_url: str, session_name: str = "") -> Tuple[Result, str]:
    """Blocking version of :py:func:`omni.client.live_create_session_with_callback`"""

    ret = None

    def live_create_session_cb(result, session_url):
        nonlocal ret
        ret = (result, session_url)

    request = live_create_session_with_callback(base_layer_url=base_layer_url, session_name=session_name, callback=live_create_session_cb)
    request.wait()
    return ret


async def live_create_session_async(base_layer_url: str, session_name: str = "") -> Tuple[Result, str]:
    """Asynchronous version of :py:func:`omni.client.live_create_session_with_callback`"""

    loop = asyncio.get_event_loop()
    f = loop.create_future()

    def live_create_session_cb(result, session_url):
        loop.call_soon_threadsafe(set_result_if_not_done, f, (result, session_url))
...
def live_create(url: str, content: bytes) -> Tuple[Result, int, int]:
    """Blocking version of :py:func:`omni.client.live_create_with_callback`"""

    ret = None

    def live_create_cb(result, object_id, sequence_num):
        nonlocal ret
        ret = (result, object_id, sequence_num)

    request = live_create_with_callback(url=url, content=content, callback=live_create_cb)
    request.wait()
    return ret


async def live_create_async(url: str, content: bytes) -> Tuple[Result, int, int]:
    """Asynchronous version of :py:func:`omni.client.live_create_with_callback`"""

    loop = asyncio.get_event_loop()
    f = loop.create_future()

    def live_create_cb(result, object_id, sequence_num):
        loop.call_soon_threadsafe(set_result_if_not_done, f, (result, object_id, sequence_num))
...
def live_update(url: str, object_id: int, content: bytes) -> Tuple[Result, int, int]:
    """Blocking version of :py:func:`omni.client.live_update_with_callback`"""

    ret = None

    def live_update_cb(result, sequence_num, request_id):
        nonlocal ret
        ret = (result, sequence_num, request_id)

    request = live_update_with_callback(url=url, object_id=object_id, content=content, callback=live_update_cb)
    request.wait()
    return ret


async def live_update_async(url: str, object_id: int, content: bytes) -> Tuple[Result, int, int]:
    """Asynchronous version of :py:func:`omni.client.live_update_with_callback`"""

    loop = asyncio.get_event_loop()
    f = loop.create_future()

    def live_update_cb(result, sequence_num, request_id):
        loop.call_soon_threadsafe(set_result_if_not_done, f, (result, sequence_num, request_id))
...
def live_send_message(join_request_id: int, content: bytes) -> Result:
    """Blocking version of :py:func:`omni.client.live_send_message_with_callback`"""

    ret = None

    def live_send_message_cb(result):
        nonlocal ret
        ret = result

    request = live_send_message_with_callback(join_request_id=join_request_id, content=content, callback=live_send_message_cb)
    request.wait()
    return ret


async def live_send_message_async(join_request_id: int, content: bytes) -> Result:
    """Asynchronous version of :py:func:`omni.client.live_send_message_with_callback`"""
...
def lock(url: str) -> Result:
    """Blocking version of :py:func:`omni.client.lock_with_callback`"""
    ...
    request = lock_with_callback(url=url, callback=lock_cb)
    request.wait()
    return ret


async def lock_async(url: str) -> Result:
    """Asynchronous version of :py:func:`omni.client.lock_with_callback`"""
...
def unlock(url: str) -> Result:
    """Blocking version of :py:func:`omni.client.unlock_with_callback`"""

    ret = None

    def unlock_cb(result):
        nonlocal ret
        ret = result

    request = unlock_with_callback(url=url, callback=unlock_cb)
    request.wait()
    return ret


async def unlock_async(url: str) -> Result:
    """Asynchronous version of :py:func:`omni.client.unlock_with_callback`"""
...
def register_storage_from_discovery(discovery_url: str) -> Tuple[Result, List[str]]:
    """Blocking version of :py:func:`omni.client.register_storage_from_discovery_with_callback`"""

    ret = None

    def register_storage_from_discovery_cb(result, addresses):
        nonlocal ret
        ret = (result, addresses)

    request = register_storage_from_discovery_with_callback(discovery_url=discovery_url, callback=register_storage_from_discovery_cb)
    request.wait()
    return ret


async def register_storage_from_discovery_async(discovery_url: str) -> Tuple[Result, List[str]]:
    """Asynchronous version of :py:func:`omni.client.register_storage_from_discovery_with_callback`"""

    loop = asyncio.get_event_loop()
    f = loop.create_future()

    def register_storage_from_discovery_cb(result, addresses):
        loop.call_soon_threadsafe(set_result_if_not_done, f, (result, addresses))
...
def register_storage_direct(url: str, headers: dict | None = None) -> Tuple[Result, List[str]]:
    """Blocking version of :py:func:`omni.client.register_storage_direct_with_callback`"""

    ret = None

    def register_storage_direct_cb(result, addresses):
        nonlocal ret
        ret = (result, addresses)

    request = register_storage_direct_with_callback(url=url, headers=headers, callback=register_storage_direct_cb)
    request.wait()
    return ret


async def register_storage_direct_async(url: str, headers: dict | None = None) -> Tuple[Result, List[str]]:
    """Asynchronous version of :py:func:`omni.client.register_storage_direct_with_callback`"""
