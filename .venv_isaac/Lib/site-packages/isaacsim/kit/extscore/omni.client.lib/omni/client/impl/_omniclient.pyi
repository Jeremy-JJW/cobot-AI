...
__all__ = [
    ...
    "list_with_callback",
    "live_create_session_with_callback",
    "live_create_with_callback",
    "live_get_latest_server_time",
    "live_join_session_with_callback",
    "live_list_sessions_with_callback",
    "live_process",
    "live_process_up_to",
    "live_read_with_callback",
    "live_register_queued_callback",
    "live_send_message_with_callback",
    "live_set_queued_callback",
    "live_update_with_callback",
...
class ItemFlags():
    ...
    Members:
      ...
      IS_CHANNEL : Deprecated: channels are deprecated and must not be used. Use live session APIs instead.
...
class LiveSessionEvent():
    """
    Event data for live session callbacks.

    This object contains information about an event that occurred in a live session,
    such as a user joining, leaving, or sending a message.
    """
    @property
    def app_name(self) -> str:
        """
        Application name (for JOINED event, empty otherwise)
    ...
    def connection_id(self) -> str:
        ...
        """
    @property
    def event_type(self) -> LiveSessionEventType:
        """
        Type of the event (CONNECTION, JOINED, LEFT, or MESSAGE)
    ...
    def user_name(self) -> str:
        """
        User name (for JOINED event, empty otherwise)
...
class LiveSessionEventType():
    """
                Event types for live session callbacks.

                These types indicate the nature of events received when joined to a live session.
            

    Members:

      CONNECTION : Successfully connected and joined the session

      JOINED : Another user joined the session

      LEFT : Another user left the session

      MESSAGE : Received a message from another user
    """
    def __eq__(self, other: object) -> bool: ...
    def __getstate__(self) -> int: ...
    def __hash__(self) -> int: ...
    ...
    def value(self) -> int:
        ...
        :type: int
        """
    CONNECTION: omni.client.impl._omniclient.LiveSessionEventType # value = <LiveSessionEventType.CONNECTION: 0>
    JOINED: omni.client.impl._omniclient.LiveSessionEventType # value = <LiveSessionEventType.JOINED: 1>
    LEFT: omni.client.impl._omniclient.LiveSessionEventType # value = <LiveSessionEventType.LEFT: 2>
    MESSAGE: omni.client.impl._omniclient.LiveSessionEventType # value = <LiveSessionEventType.MESSAGE: 3>
    __members__: dict # value = {'CONNECTION': <LiveSessionEventType.CONNECTION: 0>, 'JOINED': <LiveSessionEventType.JOINED: 1>, 'LEFT': <LiveSessionEventType.LEFT: 2>, 'MESSAGE': <LiveSessionEventType.MESSAGE: 3>}
    pass
class LiveSessionInfo():
    """
    Information about a live session.

    This object contains metadata about a live session, including its URL,
    name, owner, and when it was last modified.
    """
    @property
...
class LiveUserInfo():
    ...
    This object contains metadata about a user who is (or was) in the session.
    """
    @property
    def app_name(self) -> str:
        """
...
def bypass_list_cache(bypass: bool) -> None:
    ...
    """
def clear_list_cache(url: str) -> bool:
    """
    Clear the list/stat cache for the provider that serves the given URL.

    For HTTP/HTTPS providers, list and stat results are cached. After external changes
    to the content, call this so subsequent list/stat calls see updated results.

    Args:
        url: A URL served by the provider whose cache should be cleared (e.g. "https://example.com/container")
...
def get_version() -> str:
    ...
    """
def initialize(version: int = 563254896099330) -> bool:
    """
...
def join_channel_with_callback(url: str, callback: typing.Callable[[Result, ChannelEvent, str, Content], None]) -> Request:
    ...
    Deprecated: channels are deprecated and must not be used. Use
    :py:func:`omni.client.live_join_session_with_callback` instead.
    ...
    Note:
        ...
        leave the channel. Use :py:func:`omni.client.live_send_message_with_callback` for live session messaging.
    """
...
def list_with_callback(url: str, callback: typing.Callable[[Result, typing.List[ListEntry]], None], include_deleted_option: ListIncludeOption = ListIncludeOption.NO_DELETED_FILES) -> Request:
    ...
    """
def live_create_session_with_callback(base_layer_url: str, session_name: str, callback: typing.Callable[[Result, str], None]) -> Request:
    """
    Create a new live session.

    This function creates a new live session for collaborative editing of a USD layer.
    The session URL is assigned by the system and may be predictable (Nucleus)
    or unpredictable (Storage API backends).

    Args:
        base_layer_url: URL of the base USD layer.
        session_name: User-defined session name (must be unique per base layer).
        callback: Callback to be called with the results. The callback receives:

            - result: Result indicating success or failure
            - session_url: System-assigned session URL (empty if failed)
    ...
    Example::
        ...
        def on_create(result, session_url):
            if result == omni.client.Result.OK:
                print(f"Session created: {session_url}")

        request = omni.client.live_create_session_with_callback(
            "omniverse://localhost/Projects/my_layer.usd",
            "my_session",
            on_create)
        request.wait()
    """
def live_create_with_callback(url: str, content: buffer, callback: typing.Callable[[Result, int, int], None]) -> Request:
    """
    Create a live object.

    This function creates the live object at the specified URL using the provided content.

    Args:
        url: URL of the live object to create.
        content: Content buffer for the live object.
        callback: Callback to be called with the results. The callback receives:
                 - result: Result indicating success or failure
                 - object_id: Live object ID assigned by the server
                 - sequence_num: Sequence number assigned by the server
...
def live_get_latest_server_time(url: str) -> int:
    ...
    """
def live_join_session_with_callback(session_url: str, app_name: str, callback: typing.Callable[[Result, LiveSessionEvent], None]) -> Request:
    """
    Join a live session and receive events.

    This function joins a live session and registers a callback to receive events
    such as other users joining/leaving and messages being sent. The callback will
    be invoked multiple times as events occur.

    Args:
        session_url: Full URL of the live session to join.
        app_name: Name of your application.
        callback: Callback to be called for each event. The callback receives:
                 - result: Result indicating success or failure
                 - event: LiveSessionEvent object containing event details

    Returns:
        Request object that must be kept alive for the duration of the session.
        Call request.stop() to leave the session.

    Event Types:
        - CONNECTION: Contains your connection_id. May be delivered more than once
                 if the stream reconnects; each delivery may carry a different
                 connection_id.
        - JOINED: Contains connection_id, user_name, and app_name of the joined user.
                 Not re-delivered for peers that survive a reconnect.
        - LEFT: Contains connection_id and user_name of the user who left.
                 Emitted for peers that departed during a disconnect.
        - MESSAGE: Contains connection_id, user_name, and message_data
    """
def live_list_sessions_with_callback(base_layer_url: str, callback: typing.Callable[[Result, typing.List[LiveSessionInfo]], None]) -> Request:
    """
    List all live sessions in a folder.

    This function retrieves a list of all active live sessions in the specified folder.

    Args:
        base_layer_url: URL of a folder to list sessions in.
        callback: Callback to be called with the results. The callback receives:

            - result: Result indicating success or failure
            - sessions: List of LiveSessionInfo objects
    ...
    Example::
        ...
        def on_list(result, sessions):
            if result == omni.client.Result.OK:
                for session in sessions:
                    print(f"Session: {session.session_name}, owner: {session.session_owner}")

        request = omni.client.live_list_sessions_with_callback(
            "omniverse://localhost/Projects/base_layer.usd",
            on_list)
        request.wait()
    """
...
def live_process_up_to(url: str, server_time: int) -> None:
    ...
    """
def live_read_with_callback(*args, **kwargs) -> typing.Any:
    """
    Subscribe to a live object for updates.

    This function monitors the live object and invokes the callback when it changes.
    The callback may be invoked multiple times as the live object is updated.

    Args:
        url: URL of the live object to read.
        have_object_id: The object ID you already have, or 0 if none.
        have_sequence_num: The sequence number you already have, or 0 if none.
        callback: Callback to be called with the results. The callback receives:
                 - result: Result indicating success or failure
                 - object_id: Live object ID assigned by the server
                 - sequence_num: Sequence number assigned by the server
                 - content: Content buffer for the live object
        process_immediately: If True (default), updates are processed immediately
                 via the async callback queue. If False, updates are deferred and
                 must be driven by :py:func:`omni.client.live_process`.
...
def live_register_queued_callback(callback: typing.Callable[[LiveUpdateType, Result, str, int, int, int], None]) -> Registration:
    ...
    """
def live_send_message_with_callback(join_request_id: int, content: buffer, callback: typing.Callable[[Result], None]) -> Request:
    """
    Send a message to all users in a live session.

    This function sends a message to all other users currently joined to the same
    live session. You must be joined to the session (via live_join_session) to
    send messages.

    Args:
        join_request_id: The session ID from live_join_session (request.id).
        content: The message content to send (string or bytes).
        callback: Callback to be called with the result (Result code only).

    Returns:
        Request object that can be used to wait for completion or cancel the operation.

    Note:
        The message ownership is transferred to the client library, which will
        free it when the send operation completes.
    ...
    Example::
        ...
        request = omni.client.live_send_message_with_callback(
            join_request.id,
            message,
            on_send)
        request.wait()
...
def live_set_queued_callback(callback: typing.Callable[[], None]) -> None:
    ...
    """
def live_update_with_callback(url: str, object_id: int, content: buffer, callback: typing.Callable[[Result, int, int], None]) -> Request:
    """
    Update a live object.

    This function sends an update to the live object at the specified URL.

    Args:
        url: URL of the live object to update.
        object_id: The object ID of the live object to update.
        content: Content buffer for the update.
        callback: Callback to be called with the results. The callback receives:
                 - result: Result indicating success or failure
                 - sequence_num: Sequence number assigned by the server
                 - request_id: Request ID (reserved)
...
def register_device_flow_auth_callback(callback: typing.Callable[[int, AuthDeviceFlowParams], None]) -> Registration:
    ...
    The environment variable ``OMNICLIENT_NO_BROWSER`` (set to ``1``) also selects device flow without launching
    a browser. Without that variable, the library attempts to launch a browser first (skipping the attempt on
    Linux when ``DISPLAY`` and ``WAYLAND_DISPLAY`` are both unset) and falls back to device flow automatically
    if the launch fails. If no callback is registered, the library logs the verification URL and user code
    instead.
...
def send_message_with_callback(join_request_id: int, content: buffer, callback: typing.Callable[[Result], None]) -> Request:
    ...
    Deprecated: channels are deprecated and must not be used. Use
    :py:func:`omni.client.live_send_message_with_callback` with
    :py:func:`omni.client.live_join_session_with_callback` instead.
...
def set_log_callback(callback: typing.Callable[[str, str, LogLevel, str], None]) -> None:
    ...
    By default, messages at Info level and above are written to stderr. Call this to replace
    that default, or pass None to silence logging entirely.
...
def set_retries(max_ms: int, base_ms: int, jitter_ms: int) -> tuple:
    ...
    """
def set_s3_configuration(url: str, bucket: str = None, region: str = None, accessKeyId: str = None, secretAccessKey: str = None, sessionToken: str = None, cloudfrontUrl: str = None, cloudfrontForList: bool = False, writeConfig: bool = False, canonicalHost: str = None) -> Result:
    """
    ...
    Args:
        ...
        writeConfig: Whether to write configuration to disk (optional)
        canonicalHost: The canonical S3 host for AWS signature (e.g. bucket.s3.region.amazonaws.com). Use when accessing S3 via custom domain. (optional)
...
def unlock_with_callback(url: str, callback: typing.Callable[[Result], None]) -> Request:
    ...
    """
def write_chrome_trace() -> None:
    """
    Write Chrome trace events to file and reset the trace buffer.

    This function writes all accumulated Chrome trace events to the file specified by
    the OMNICLIENT_ENABLE_CHROME_TRACING environment variable, then clears the trace buffer
    and resets the timing. This allows you to periodically flush traces without having
    to wait for shutdown.

    Note:
        Chrome tracing is only active if the OMNICLIENT_ENABLE_CHROME_TRACING environment
        variable is set to a file path (e.g., "/path/to/trace.json"). If the variable is not set
        or is empty, this function does nothing.
