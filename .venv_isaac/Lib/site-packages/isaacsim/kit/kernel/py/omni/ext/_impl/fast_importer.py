...
def _refresh_cache():
    """Rebuilds or updates the importer cache based on current sys paths.

    Loads from disk if enabled, scans new paths, updates namespaces/modules,
    and persists the cache when file caching is turned on.
    """
    ...
    # If an extension is shutdown, path is removed from cache, also remove cache module/path related.
    # Otherwise, when loading same module from different path/version with leads to cached module/path.
    removed_paths = old_paths - paths
    for path in removed_paths:
        path = path.replace("\\", "/")
        _cache.modules = {
            module: _cache.modules[module] for module in _cache.modules if not _cache.modules[module].startswith(path)
        }
...
def _fast_join(p0, p1):
    """Fast string join for paths using '/' separator."""
    return f"{p0}/{p1}"


def _fast_path_split(path):
    """Fast path split returning (dirname, basename) using '/' separator."""
    i = path.rfind("/") + 1
...
class FastFinder:
    ...
    def find_distributions(cls, context):
        """Provides package metadata distributions for modules discovered by fast importer."""
