...
def _is_in_folder(path, parent):
    """Case-insensitive check whether `path` is inside `parent` (absolute)."""
...
def _is_subpath(path, parent):
    ...
    """Pathlib-based check whether `path` is a descendant of `parent`."""
    return Path(parent) in Path(path).parents


def _is_module_in_folder(module: str, parent: str):
    """Returns True if imported module's file resides under `parent` folder."""
...
def find_modules_in_folder(folder: str):
    """Returns a list of loaded module names whose files reside under `folder`."""
    modules = []
...
def _import_public(folder, module_name: str, reload_enabled: bool = True):
    """Imports or reloads a public module, warning on name clashes across extensions."""
...
def _multi_startup_check(ext_obj, path, module_name):
    """Warns if an IExt subclass is imported from a different extension's path (Windows only)."""
...
class ExtensionModule:
    """Descriptor of a Python module that belongs to an extension."""
...
class ExtensionModules:
    """Manages Python module lifecycle for a single extension (startup/shutdown)."""
    ...
    def _startup_ext(self, ext_class: Type[IExt], module_name: str, path: str):
        """Instantiates an IExt subclass and invokes its on_startup hook."""
    ...
    def _add_sys_path(self, path, module_name):
        """Adds a path for public modules either to fast importer or sys.path."""
        is_first = False
    ...
    def _custom_importer(self, ext_module: ExtensionModule, reload_enabled: bool = True):
        """Imports a module using public or private import path logic; logs failures."""
        try:
    ...
    def prestartup_module(self, ext_module: ExtensionModule):
        """Registers a module to be imported on extension startup."""
    ...
    def startup(self):
        """Imports all registered modules and starts discovered IExt implementations."""
    ...
    def shutdown(self):
        """Stops started IExt implementations and cleans import/system path state."""
...
def prestartup_all_extensions_in_module(
    ...
    """Registers a module for an extension prior to startup.

    Args:
        ext_id: Extension identifier.
        path: Filesystem path where the module is located.
        module_name: Public or private module name to import.
        public: If True, add path to sys.path/fast importer for public import.
        search_ext: If True, search the module for IExt subclasses to start.
    """
    _initialize_once()
...
def startup_extension(ext_id: str):
    """Starts all registered modules for an extension, importing and invoking IExt.on_startup."""
    global _extensions
    if ext_id not in _extensions:
        return
...
def shutdown_extension(ext_id: str):
    """Stops a started extension, invoking IExt.on_shutdown and cleaning imports/paths."""
    global _extensions
    if ext_id not in _extensions:
        return
...
def get_ext_id_from_module(module_name: str) -> str:
    """Returns the extension id that provided a given module name, if any."""
