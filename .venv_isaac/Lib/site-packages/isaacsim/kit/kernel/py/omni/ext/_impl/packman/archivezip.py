...
# [per file] data descriptor = max 16
# [per file] central dire header = 46 + len(filename)
# EOCD = 22
# (76 + (2 * max_file_len) + 16) + 22
# file_count * (92 + (2 * max_file_len)) + 22 = rough_max_zip_overhead
Z_HEADER = 92
Z_EOCD = 22
MIN_ZIP = 22
MAX_FILE_NAME_LEN = 4095
MAX_FILE_CNT = 65535
...
def make_archive_from_folder(input_folder: str, archive_path: str, store_links=False, store_file_links=False) -> str:
    ...
    return make_archive_from_file_list(
        archive_path, file_list, store_links=store_links, store_file_links=store_file_links
    )


def make_archive_from_file_list(package_path, iterable_file_list, store_links=False, store_file_links=False):
    ...
    manifest = archive.create_manifest(
        iterable_file_list, store_links, encode_links=True, store_file_links=store_file_links
    )
...
def _extract_link(zip: zipfile.ZipFile, file_info: zipfile.ZipInfo, extract_dir: str):
    ...
    try:
        target_path = target_path.decode("utf8", errors="ignore")
...
def _validate_archive(archive_path: str) -> bool:
    """
    Defensive checks to determine archive is well formed and
    safe enough to extract.
    """
    zip_size = 0
    uncompress_size = 0
    compress_size = 0
    ...
    # 0. Get metadata from archive for validation checks
    try:
        zip_size = os.path.getsize(archive_path)
    except OSError:
        logger.error(f"File {archive_path} does not exists or is inaccessible")
        return False

    with zipfile.ZipFile(archive_path, allowZip64=True) as zip_file:
        file_count = len(zip_file.infolist())
        for file in zip_file.infolist():
            uncompress_size += file.file_size
            compress_size += file.compress_size
            max_name_len = max(max_name_len, len(file.filename))

    # 1. Check for multi-part, malformed and malicious zip structures
    if file_count > MAX_FILE_CNT:
        logger.error(f"File {archive_path} file count exceeds max. count: {file_count}")
        return False
    elif zip_size < MIN_ZIP:
        logger.error(f"File {archive_path} is below smallest zip size. size: {zip_size}")
        return False
    elif max_name_len > MAX_FILE_NAME_LEN:
        logger.error(f"File {archive_path} has a file name which exceeds max")
        return False

    overhead = file_count * ((2 * max_name_len) + Z_HEADER) + Z_EOCD

    if (overhead + compress_size) < zip_size:
        logger.error(f"File {archive_path} has mal multi-part structure or odd optional data")
        return False

    # 2. Check for zip bombs and except out
    archive.raise_if_insufficient_space_for_decompression(archive_path, uncompress_size)

    # 3. Zipslip attacks handled by extract in standard python zip library
    return True


def extract_archive_to_folder(archive_path, output_folder, trusted=True):
    """
    Extracts the zip archive at 'archive_path' to folder 'output_folder'.
    """

    # if registry archive is not trusted do extra security checks
    if not trusted and _validate_archive(archive_path) == False:
        return
