...
)
{
    return math::lerp(
        constant_roughness,
        get_float(roughness_texture, constant_roughness, uvw),
        roughness_influence
    );
}
...
= let {
    // Material Properties
    bool local_thin_walled = (tex::texture_isvalid(cutout_opacity_texture) || cutout_opacity != 1.f) ? true : thin_walled;
    ...
    // Opacity Properties
    float opacity_value = get_float(cutout_opacity_texture, cutout_opacity, transformed_uvw, cutout_opacity_mono_source);
    ...
    float local_cutout_opacity = enable_opacity ?
    ...
    // Surface Properties
    color local_reflection_color = get_color(reflection_color_texture, reflection_color, transformed_uvw);
    color local_transmission_color = get_color(glass_color_texture, glass_color, transformed_uvw);

    OmniPBRBase::NormalData defaultData(state::normal(), float2(0.f));
    OmniPBRBase::NormalData normal_data = tex::texture_isvalid(normal_map_texture) ?
    ...
    float2 local_roughness_squared(
        (local_roughness * local_roughness +
        geometry_normal_roughness_strength * normal_data.roughness.x * normal_data.roughness.x),
        (local_roughness * local_roughness +
        geometry_normal_roughness_strength * normal_data.roughness.y * normal_data.roughness.y)
    );

    float3 the_normal = normal_data.normal;

    bsdf ggx_1 = ::df::microfacet_ggx_smith_bsdf(
        roughness_u: local_roughness_squared.x,
        roughness_v: local_roughness_squared.y,
        tint: local_reflection_color,
    ...
    material_volume local_volume(vdf(), local_volume_absorption, local_volume_scattering);


    // Geometry Properties
    material_geometry local_geometry(float3(0.f), local_cutout_opacity, the_normal);
