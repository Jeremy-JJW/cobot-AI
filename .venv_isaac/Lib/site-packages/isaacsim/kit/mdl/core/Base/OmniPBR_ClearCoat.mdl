...
export material OmniPBR_ClearCoat(
	...
	float albedo_desaturation = float(0.0) [[
		anno::display_name("Albedo Map Desaturation"),
		anno::description("Desaturates the albedo map"),
		anno::in_group("Albedo"),
	...
	float emissive_intensity = float(40.0) [[
	...
	float opacity_constant = float(1.0) [[
	...
	uniform float2 clearcoat_texture_translate =  float2(0.0) [[
		...
		anno::description("Controls the position of the clearcoat texture"),
		anno::ui_order(1),
		anno::in_group("UV")
	]],
	uniform float clearcoat_texture_rotate = float(0.0) [[
		anno::display_name("Clearcoat Texture Rotate"),
		anno::description("Rotates angle of the clearcoat texture in degrees"),
		anno::ui_order(2),
		anno::in_group("UV")
	]],
	uniform float2 clearcoat_texture_scale = float2(1.0) [[
		anno::display_name("Clearcoat Texture Tiling"),
		anno::description("Controls the repetition of the clearcoat texture"),
		anno::ui_order(3),
		anno::in_group("UV")
	]],
	...
	uniform float geometry_normal_roughness_strength = 1.f
	[[
		anno::hard_range(0.0, 1.0),
		anno::display_name("Normal map to roughness weight"),
		anno::description("Enables and weights roughness induced by normal maps"),
	...
	uniform float2 detail_texture_scale = float2(1.0) [[
		anno::display_name("Detail Texture Tiling"),
		anno::description("Controls repetition of the detail texture."),
		anno::in_group("UV"),
		anno::enable_if("project_uvw == true")
    ]],
...
[[
	...
	//  Normal calculations  //
	OmniPBRBase::NormalData defaultData = OmniPBRBase::NormalData(state::normal(), float2(0));

	OmniPBRBase::NormalData normal_data = tex::texture_isvalid(normalmap_texture) ?
		OmniPBRBase::tangent_space_normal_roughness_texture(
	...
	float3 the_normal = normal_data.normal;

	OmniPBRBase::NormalData detail_normal_data = tex::texture_isvalid(detail_normalmap_texture) ?
		OmniPBRBase::tangent_space_normal_roughness_texture(
	...
	float3 detail_normal = detail_normal_data.normal;

	OmniPBRBase::NormalData clearcoat_normal_data = tex::texture_isvalid(clearcoat_normalmap_texture) ?
		OmniPBRBase::tangent_space_normal_roughness_texture(
	...
	float3 clearcoat_normal = clearcoat_normal_data.normal;
	...
	float reflection_roughness_s   = math::lerp(reflection_roughness_constant, roughness_selection, reflection_roughness_texture_influence);
	float2 reflection_roughness = float2(math::sqrt(reflection_roughness_s*reflection_roughness_s + (geometry_normal_roughness_strength * normal_data.roughness.x * normal_data.roughness.x)),
	                                     math::sqrt(reflection_roughness_s*reflection_roughness_s + (geometry_normal_roughness_strength * normal_data.roughness.y * normal_data.roughness.y)));

	float2 clearcoat_reflection_roughness2 = float2(math::sqrt(clearcoat_reflection_roughness*clearcoat_reflection_roughness + (geometry_normal_roughness_strength * clearcoat_normal_data.roughness.x * clearcoat_normal_data.roughness.x)),
												    math::sqrt(clearcoat_reflection_roughness*clearcoat_reflection_roughness + (geometry_normal_roughness_strength * clearcoat_normal_data.roughness.y * clearcoat_normal_data.roughness.y)));
	...
	material base = OmniPBRBase::OmniPBRBase(
		...
		specular_reflection_aniso_roughness: reflection_roughness, 
		...
		coat_aniso_roughness: clearcoat_reflection_roughness2,
