...
rtx.hydra.supportMultiTickRate = true # enable multi-tick rendering so each sensor can render at its own tick rate
rtx.rendering.perSensorTickTlas = true # per-sensor TLAS updates for multi-tick rendering
rtx.post.dlss.execMode = 0 # Set the DLSS model can be 0 (Performance), 1 (Balanced), 2 (Quality), or 3 (Auto)
...
runLoops.main.rateLimitEnabled = false           # Disable main run-loop rate limit (no end-of-frame sleep); loop ticks as fast as the system allows
runLoops.main.manualModeEnabled = true           # Enable manual mode, dt is fixed at requested rate
player.useFixedTimeStepping = false              # Disable fixed time stepping; timeline advances by the loop runner's wall-clock dt (or manual_step_size when manual_mode is on)
...
app.viewport.camVelocityMin = 0.0001 # Meters default
app.viewport.camVelocityMax = 0.2 # Meters default
