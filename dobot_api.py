...
alarmControllerFile = "./alarmController.json"
alarmServoFile = "./alarmServo.json"
...
class DobotApiDashboard(DobotApi):
    ...
    def MovLIO(self, a1, b1, c1, d1, e1, f1, coordinateMode, Mode, Distance, Index, Status, user=-1, tool=-1, a=-1,
               v=-1, speed=-1, cp=-1, r=-1):
        """
        描述
    ...
    def MovJIO(self, a1, b1, c1, d1, e1, f1, coordinateMode, Mode, Distance, Index, Status, user=-1, tool=-1, a=-1,
               v=-1, cp=-1, ):
        """
        描述
    ...
    def Arc(self, a1, b1, c1, d1, e1, f1, a2, b2, c2, d2, e2, f2, coordinateMode, user=-1, tool=-1, a=-1, v=-1,
            speed=-1, cp=-1, r=-1):
        """
        描述
        从当前位置以圆弧插补⽅式运动⾄⽬标点。
    ...
    def Circle(self, a1, b1, c1, d1, e1, f1, a2, b2, c2, d2, e2, f2, coordinateMode, count, user=-1, tool=-1, a=-1,
               v=-1, speed=-1, cp=-1, r=-1):
        """
        描述
    ...
    def RelMovJTool(self, offset_x, offset_y, offset_z, offset_rx, offset_ry, offset_rz, user=-1, tool=-1, a=-1, v=-1,
                    cp=-1):
        """
        描述
    ...
    def RelMovLTool(self, offset_x, offset_y, offset_z, offset_rx, offset_ry, offset_rz, user=-1, tool=-1, a=-1, v=-1,
                    speed=-1, cp=-1, r=-1):
        """
        描述
    ...
    def RelMovJUser(self, offset_x, offset_y, offset_z, offset_rx, offset_ry, offset_rz, user=-1, tool=-1, a=-1, v=-1,
                    cp=-1):
        """
        描述
    ...
    def RelMovLUser(self, offset_x, offset_y, offset_z, offset_rx, offset_ry, offset_rz, user=-1, tool=-1, a=-1, v=-1,
                    speed=-1, cp=-1, r=-1):
        """
        描述
    ...
    def ParseResultId(self, valueRecv):
        ...
        """
        if valueRecv.find(
                "Not Tcp") != -1:  # 通过返回值判断机器是否处于tcp模式 Judge whether the robot is in TCP mode by the return value
    ...
    def FCForceMode(self, x, y, z, rx, ry, rz, fx, fy, fz, frx, fry, frz, reference=-1, user=-1, tool=-1):
        ...
        """
        string = ""
        string = "FCForceMode(" + "{" + "{:d},{:d},{:d},{:d},{:d},{:d}".format(x, y, z, rx, ry,
                                                                               rz) + "}," + "{" + "{:d},{:d},{:d},{:d},{:d},{:d}".format(
            fx, fy, fz, frx, fry, frz) + "}"
        params = []
        if reference != -1:
    ...
    def RelPointTool(self, J1, J2, J3, J4, J5, J6, x, y, z, rx, ry, rz):
        ...
        """
        string = ""
        string = "RelPointTool(" + "{:f},{:f},{:f},{:f},{:f},{:f}".format(J1, J2, J3, J4, J5,
                                                                          J6) + "{" + "{:f},{:f},{:f},{:f},{:f},{:f}".format(
            x, y, z, rx, ry, rz) + "}"
        string = string + ')'
        return self.sendRecvMsg(string)
