...
| 文件 | 作用 |
|------|------|
| `script_parser.py` | 脚本指令解析：一句话 → 一/多条 IR（含 DI 条件、程序点、DO/等待） |
| `program_runner.py` | 在线逐行执行整段程序（DI 条件判断、程序点解析、session 保活） |
| `lua_exporter.py` | 程序 IR → 越疆 Lua5 脚本文本（点定义 + 运动 + IO + 条件） |
| `lua_commands.md` | 可用 Lua 指令白名单与 IR→Lua 映射表 |
| `二期改造完整方案规划说明.md` | 二期完整方案（布局、交互、阶段任务） |
| `programs/` | 本地脚本程序存储目录（运行时自动创建，每个 `.json` 一个程序） |
| 新 Skill | `skills/move_to_program_point.py`、`skills/set_do.py`、`skills/wait_di.py` |
| 新增 API | `/api/script/plan`、`/api/capture-pose`、`/api/programs[/<name>]`、`/api/run-program`、`/api/export-lua` |

前端：`templates/index.html` 增加左侧「对话控制 / 脚本编程」侧边栏；`static/js/app.js`、`static/css/app.css` 新增脚本编辑器（指令列表、存点列表、语音插入、保存/导入/导出 Lua/运行）。
